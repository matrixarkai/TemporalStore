...
#define SHARED_FILE_STORE_INJECT_ACTION(name, api, path)
#define SHARED_FILE_STORE_INJECT_ACTION_WITH_TYPE(name, api, path, type)
#define SHARED_FILE_STORE_FAULT_INJECT(name, api, path)
...
#define SHARED_FILE_STORE_INJECT_ACTION_WITH_TYPE(name, api, path, type) \
    do {                                                                 \
        if (std::string(path).find(type) != std::string::npos) {         \
            SHARED_FILE_STORE_INJECT_ACTION(name, api, path);            \
        }                                                                \
    } while (false);

#define SHARED_FILE_STORE_FAULT_INJECT(name, api, path)                           \
    do {                                                                         \
        fiu_do_on(name, SHARED_FILE_STORE_INJECT_ACTION(name, api, path));        \
        fiu_do_on((std::string("index/") + name).data(),                         \
                  SHARED_FILE_STORE_INJECT_ACTION_WITH_TYPE(name, api, path,      \
                                                            "index"));           \
...
std::string StripPrefix(const std::string& uri, const std::string& prefix) {
...
std::string GetFile(const std::string& uri) {
    std::string file = StripPrefix(uri, "shared-file://");
    if (!file.empty()) {
        return file;
    }
    file = StripPrefix(uri, "shared://");
    if (!file.empty()) {
        return file;
    }
    file = StripPrefix(uri, "efs://");
    if (!file.empty()) {
        return file;
    }
    return StripPrefix(uri, "nfs://");
}
...
SharedFileCondition::SharedFileCondition(const Store::Condition& condition)
    : uri_(condition.name), data_(condition.data) {}

Status SharedFileCondition::Check() {
    if (uri_.empty()) {
        return Status::OK();
    }
    ...
    int fd = open(file.c_str(), O_RDONLY | O_SYNC, 0444);
    if (fd < 0) {
        if (errno == ENOENT) {
    ...
    if (s.st_size > 0) {
        ssize_t io_size = pread(fd, &data[0], s.st_size, 0);
        if (io_size != s.st_size) {
            LOG_ERROR("Pread file failed").put("File", file).put("Error", strerror(errno));
            close(fd);
            return Status::StoreInternal(strerror(errno));
        }
    }
...
void SharedFile::Append(Controller* ctrl, const void* data, size_t size,
...
void SharedFile::Read(Controller* ctrl, size_t offset, void* data, size_t size,
                      Closure<void>* callback) {
    ScopedInvoker done(callback);
...
#define SHARED_CONDITION_GUARD(condition)                                           \
    SharedFileCondition condition_guard(condition);                                 \
    Status _status = condition_guard.Check();                                       \
...
void SharedFileStore::SetCondition(Controller* ctrl, const std::string& uri,
                                   const ConditionData& data,
                                   const SetConditionOptions& options) {
    SHARED_FILE_STORE_FAULT_INJECT("store/shared_file/ioctl/fail/set_condition",
                                   "SetCondition", uri);
    GET_PATHNAME(uri, file);
    SHARED_CONDITION_GUARD(options.condition);
...
void SharedFileStore::StatCondition(Controller* ctrl, const std::string& uri,
                                    ConditionData* data) {
    SHARED_FILE_STORE_FAULT_INJECT("store/shared_file/ioctl/fail/stat_condition",
                                   "StatCondition", uri);
    GET_PATHNAME(uri, file);
...
void SharedFileStore::List(Controller* ctrl, const std::string& path,
                           std::vector<BlobInfo>* files) {
    SHARED_FILE_STORE_FAULT_INJECT("store/shared_file/ioctl/fail/list", "List", path);
...
void SharedFileStore::Open(Controller* ctrl, const std::string& uri,
                           const OpenOptions& options, Blob** blob) {
    SHARED_FILE_STORE_FAULT_INJECT("store/shared_file/ioctl/fail/open", "Open", uri);
    ...
    *blob = new SharedFile(file, fd, options.metrics_manager);
    ctrl->set_status(Status::OK());
}

void SharedFileStore::Delete(Controller* ctrl, const std::string& uri,
                             const DeleteOptions& options) {
    SHARED_FILE_STORE_FAULT_INJECT("store/shared_file/ioctl/fail/delete", "Delete", uri);
    GET_PATHNAME(uri, file);
...
void SharedFileStore::Freeze(Controller* ctrl, const std::string& uri,
                             const FreezeOptions& options) {
    SHARED_FILE_STORE_FAULT_INJECT("store/shared_file/ioctl/fail/freeze", "Freeze", uri);
    GET_PATHNAME(uri, file);
...
void SharedFileStore::Stat(Controller* ctrl, const std::string& uri,
                           const StatOptions& options, BlobStat* stat_info) {
    SHARED_FILE_STORE_FAULT_INJECT("store/shared_file/ioctl/fail/stat", "Stat", uri);
    (void)options;
    GET_PATHNAME(uri, file);
...
void SharedFileStore::Rename(Controller* ctrl, const std::string& src_uri,
                             const std::string& dst_uri,
                             const RenameOptions& options) {
    SHARED_FILE_STORE_FAULT_INJECT("store/shared_file/ioctl/fail/rename", "Rename", dst_uri);
