...
/*
    This test aims at testing that the DRAM storage engine can
...
class StorageEngineDramTest : public testing::Test {
 public:
    void SetUp() override {
        storage_engine_ = new bcache2::StorageEngineDram<int, int>(bcache2::AllocatorType::Standard,
                                                                   storage_engine_cap);
    }
 ...
 protected:
    ...
    bcache2::StorageEngineDram<int, int>* storage_engine_;
};

TEST_F(StorageEngineDramTest, InsertTest) {
    ...
    // Get should return NOT_FOUND since the key does not exist
