...
class StorageEngineDram : public StorageEngine<K, V> {
    AllocatorType alloc_type_;
    CacheAllocator* allocator_;

 public:
    StorageEngineDram(AllocatorType type, size_t cap);
    virtual ~StorageEngineDram();

    /**
     * @brief get the value associated with query key, store the value (excluding key, header
     * and CRC) in the resp field
     *
     * @param key is the query key
     * @param sz is the size of the data block (including key, value, header and CRC) to be
     * loaded from the storage engine
    ...
    /**
     * @brief delete a data block (including a key, value, header and CRC) from the storage
     * engine
     *
     * @param addr is the address of the data block in the storage engine
