...
class cpc_union_alloc {
 public:
     ...
     */
    explicit cpc_union_alloc(uint8_t lg_k = cpc_constants::DEFAULT_LG_K,
 ...
 private:
    ...
    template <typename S>
    void internal_update(S&& sketch);  // to support both rvalue and lvalue
    ...
    void or_window_into_matrix(const vector_u8<A>& sliding_window, uint8_t offset,
                               uint8_t src_lg_k);
...
#include "model/cpc/cpc_union_impl.h"
