...
#include "model/cpc/common_defs.h"
#include "model/cpc/cpc_common.h"
#include "model/cpc/cpc_compressor.h"
#include "model/cpc/cpc_confidence.h"
#include "model/cpc/u32_table.h"
...
class cpc_sketch_alloc {
 public:
     ...
     */
    explicit cpc_sketch_alloc(uint8_t lg_k = cpc_constants::DEFAULT_LG_K,
                              uint64_t seed = DEFAULT_SEED, const A& allocator = A());
     ...
     */
    // NOLINTNEXTLINE(runtime/references)
    static cpc_sketch_alloc<A> deserialize(google::protobuf::io::CodedInputStream& is,
                                           uint64_t seed = DEFAULT_SEED, const A& allocator = A());
     ...
     */
    static cpc_sketch_alloc<A> deserialize(const void* bytes, size_t size,
                                           uint64_t seed = DEFAULT_SEED, const A& allocator = A());
 ...
 private:
    ...
    cpc_sketch_alloc(uint8_t lg_k, uint32_t num_coupons, uint8_t first_interesting_column,
    ...
    // NOLINTNEXTLINE(runtime/references)
    inline void write_hip(google::protobuf::io::CodedOutputStream& stream) const;
    // NOLINTNEXTLINE(runtime/references)
    static double read_double(google::protobuf::io::CodedInputStream& is);
    // NOLINTNEXTLINE(runtime/references)
    inline void write_double(google::protobuf::io::CodedOutputStream& stream,
    ...
    static uint8_t get_preamble_ints(uint32_t num_coupons, bool has_hip, bool has_table,
                                     bool has_window);
...
#include "model/cpc/cpc_sketch_impl.h"
