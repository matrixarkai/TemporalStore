...
#include "model/cpc/cpc_confidence.h"
#include "model/cpc/cpc_util.h"
#include "model/cpc/icon_estimator.h"
#include "model/cpc/inv_pow2_table.h"
#include "model/cpc/kxp_byte_lookup.h"
#include "model/cpc/serde.h"

namespace datasketches {

template <typename A>
void cpc_init() {
    get_compressor<A>();  // this initializes a global static instance of the compressor on the
                          // first use
}
...
void cpc_sketch_alloc<A>::update(double value) {
    ...
    if (value == 0.0) {
        ...
        ldu.long_value = 0x7ff8000000000000L;  // canonicalize NaN using value from Java's
...
void cpc_sketch_alloc<A>::update_sparse(uint32_t row_col) {
    ...
    if (c32pre >= 3 * k)
        throw std::logic_error("c32pre >= 3 * k");  // C < 3K/32, in other words flavor == SPARSE
...
void cpc_sketch_alloc<A>::update_windowed(uint32_t row_col) {
    ...
    const uint64_t c32pre = static_cast<uint64_t>(num_coupons) << 5;
    if (c32pre < 3 * k)
        throw std::logic_error("c32pre < 3 * k");  // C < 3K/32, in other words flavor >= HYBRID
    const uint64_t c8pre = static_cast<uint64_t>(num_coupons) << 3;
    const uint64_t w8pre = static_cast<uint64_t>(window_offset) << 3;
    if (c8pre >= (27 + w8pre) * k)
        throw std::logic_error("c8pre is wrong");  // C < (K * 27/8) + (K * window_offset)
    ...
    if (is_novel) {
        ...
        if (c8post >= (27 + w8pre) * k) {
            move_window();
            if (window_offset < 1 || window_offset > 56)
                throw std::logic_error("wrong window offset");
            const uint64_t w8post = static_cast<uint64_t>(window_offset) << 3;
            if (c8post >= (27 + w8post) * k)
                throw std::logic_error("c8pre is wrong");  // C < (K * 27/8) + (K * window_offset)
        }
    }
}
...
void cpc_sketch_alloc<A>::move_window() {
    ...
    if (new_offset != determine_correct_offset(lg_k, num_coupons))
        throw std::logic_error("new_offset is wrong");
    ...
    const uint64_t mask_for_clearing_window =
        (static_cast<uint64_t>(0xff) << new_offset) ^ UINT64_MAX;
    ...
    if (first_interesting_column > new_offset)
        first_interesting_column = new_offset;  // corner case
}
...
string<A> cpc_sketch_alloc<A>::to_string() const {
    // Using a temporary stream for implementation here does not comply with AllocatorAwareContainer
    // requirements. The stream does not support passing an allocator instance, and alternatives are
    // complicated.
    ...
    os << "   window         : " << (sliding_window.size() == 0 ? "not " : "") << "allocated"
       << std::endl;
    if (sliding_window.size() > 0) {
...
void cpc_sketch_alloc<A>::serialize(std::ostream& os) const {
    ...
    const uint8_t flags_byte((1 << flags::IS_COMPRESSED) | (has_hip ? 1 << flags::HAS_HIP : 0) |
    ...
    if (!is_empty()) {
        ...
        if (has_window) {
            write(os, compressed.window_data.data(),
                  compressed.window_data_words * sizeof(uint32_t));
        }
        if (has_table) {
...
vector_u8<A> cpc_sketch_alloc<A>::serialize(unsigned header_size_bytes) const {
    ...
    const bool has_window = compressed.window_data.size() > 0;
    const uint8_t preamble_ints = get_preamble_ints(num_coupons, has_hip, has_table, has_window);
    const size_t size = header_size_bytes + (preamble_ints + compressed.table_data_words +
    ...
    if (!is_empty()) {
        ...
        if (has_window) {
            ptr += copy_to_mem(compressed.window_data.data(), ptr,
                               compressed.window_data_words * sizeof(uint32_t));
        }
        if (has_table) {
            ptr += copy_to_mem(compressed.table_data.data(), ptr,
                               compressed.table_data_words * sizeof(uint32_t));
        }
    }
...
cpc_sketch_alloc<A> cpc_sketch_alloc<A>::deserialize(std::istream& is, uint64_t seed,
    ...
    if (has_table || has_window) {
        ...
        if (has_window) {
            ...
            read(is, compressed.window_data.data(),
                 compressed.window_data_words * sizeof(uint32_t));
        }
        if (has_table) {
    ...
    if (preamble_ints != expected_preamble_ints) {
        throw std::invalid_argument("Possible corruption: preamble ints: expected " +
                                    std::to_string(expected_preamble_ints) + ", got " +
                                    std::to_string(preamble_ints));
    }
    if (serial_version != SERIAL_VERSION) {
        throw std::invalid_argument("Possible corruption: serial version: expected " +
                                    std::to_string(SERIAL_VERSION) + ", got " +
    ...
    return cpc_sketch_alloc(lg_k, num_coupons, first_interesting_column,
                            std::move(uncompressed.table), std::move(uncompressed.window), has_hip,
                            kxp, hip_est_accum, seed);
}

template <typename A>
// NOLINTNEXTLINE(runtime/references)
void cpc_sketch_alloc<A>::serialize(google::protobuf::io::CodedOutputStream& output) const {
    compressed_state<A> compressed(A(sliding_window.get_allocator()));
    compressed.table_data_words = 0;
    compressed.table_num_entries = 0;
    compressed.window_data_words = 0;
    ...
    const bool has_window = compressed.window_data.size() > 0;
    const uint8_t preamble_ints = get_preamble_ints(num_coupons, has_hip, has_table, has_window);
    output.WriteRaw(&preamble_ints, 1);
    const uint8_t serial_version = SERIAL_VERSION;
    output.WriteRaw(&serial_version, 1);
    const uint8_t family = FAMILY;
    output.WriteRaw(&family, 1);
    output.WriteRaw(&lg_k, 1);
    output.WriteRaw(&first_interesting_column, 1);

    const uint8_t flags_byte((1 << flags::IS_COMPRESSED) | (has_hip ? 1 << flags::HAS_HIP : 0) |
                             (has_table ? 1 << flags::HAS_TABLE : 0) |
                             (has_window ? 1 << flags::HAS_WINDOW : 0));
    output.WriteRaw(&flags_byte, 1);
    const uint16_t seed_hash(compute_seed_hash(seed));
    output.WriteRaw(&seed_hash, 2);
    if (!is_empty()) {
        output.WriteVarint32(num_coupons);
        if (has_table && has_window) {
            // if there is no window it is the same as number of coupons
            output.WriteVarint32(compressed.table_num_entries);
            // HIP values can be in two different places in the sequence of fields
            // this is the first HIP decision point
            if (has_hip) {
                write_hip(output);
        ...
        if (has_table) {
            output.WriteVarint32(compressed.table_data_words);
        }
        if (has_window) {
            output.WriteVarint32(compressed.window_data_words);
        }
        // this is the second HIP decision point
        if (has_hip && !(has_table && has_window)) {
            write_hip(output);
        ...
        if (has_window) {
            output.WriteRaw(compressed.window_data.data(),
                            compressed.window_data_words * sizeof(uint32_t));
        }
        if (has_table) {
            output.WriteRaw(compressed.table_data.data(),
                            compressed.table_data_words * sizeof(uint32_t));
        }
    }
}
...
template <typename A>
cpc_sketch_alloc<A> cpc_sketch_alloc<A>::deserialize(
    // NOLINTNEXTLINE(runtime/references)
    google::protobuf::io::CodedInputStream& is, uint64_t seed, const A& allocator) {
    uint8_t preamble_ints;
    is.ReadRaw(&preamble_ints, 1);

    uint8_t serial_version;
    is.ReadRaw(&serial_version, 1);

    uint8_t family_id;
    is.ReadRaw(&family_id, 1);

    uint8_t lg_k;
    is.ReadRaw(&lg_k, 1);

    uint8_t first_interesting_column;
    is.ReadRaw(&first_interesting_column, 1);

    uint8_t flags_byte;
    is.ReadRaw(&flags_byte, 1);

    uint16_t seed_hash;
    is.ReadRaw(&seed_hash, 2);
    ...
    double hip_est_accum = 0;
    if (has_table || has_window) {
        is.ReadVarint32(&num_coupons);
        if (has_table && has_window) {
            uint32_t table_num_entries;
            is.ReadVarint32(&table_num_entries);
            compressed.table_num_entries = table_num_entries;
            if (has_hip) {
                kxp = read_double(is);
                hip_est_accum = read_double(is);
            }
        }
        if (has_table) {
            uint32_t table_data_words;
            is.ReadVarint32(&table_data_words);
            compressed.table_data_words = table_data_words;
        }

        if (has_window) {
            uint32_t window_data_words;
            is.ReadVarint32(&window_data_words);
            compressed.window_data_words = window_data_words;
        }
        if (has_hip && !(has_table && has_window)) {
            kxp = read_double(is);
            hip_est_accum = read_double(is);
        ...
        if (has_window) {
            compressed.window_data.resize(compressed.window_data_words);
            is.ReadRaw(compressed.window_data.data(),
                       compressed.window_data_words * sizeof(uint32_t));
        }
        if (has_table) {
            compressed.table_data.resize(compressed.table_data_words);
            is.ReadRaw(compressed.table_data.data(),
                       compressed.table_data_words * sizeof(uint32_t));
        }
        if (!has_window) {
...
cpc_sketch_alloc<A> cpc_sketch_alloc<A>::deserialize(const void* bytes, size_t size, uint64_t seed,
    ...
    if (has_table || has_window) {
        ...
        if (has_window) {
            ...
            ptr += copy_from_mem(ptr, compressed.window_data.data(),
                                 compressed.window_data_words * sizeof(uint32_t));
        }
        if (has_table) {
            compressed.table_data.resize(compressed.table_data_words);
            check_memory_size(ptr - base + (compressed.table_data_words * sizeof(uint32_t)), size);
            ptr += copy_from_mem(ptr, compressed.table_data.data(),
                                 compressed.table_data_words * sizeof(uint32_t));
        }
        if (!has_window) compressed.table_num_entries = num_coupons;
    }
    if (ptr != static_cast<const char*>(bytes) + size)
        throw std::logic_error("deserialized size mismatch");
...
size_t cpc_sketch_alloc<A>::get_max_serialized_size_bytes(uint8_t lg_k) {
    ...
    if (lg_k <= CPC_EMPIRICAL_SIZE_MAX_LGK)
        return CPC_EMPIRICAL_MAX_SIZE_BYTES[lg_k - CPC_MIN_LG_K] + CPC_MAX_PREAMBLE_SIZE_BYTES;
    const uint32_t k = 1 << lg_k;
    return static_cast<int>(CPC_EMPIRICAL_MAX_SIZE_FACTOR * k) + CPC_MAX_PREAMBLE_SIZE_BYTES;
}

template <typename A>
void cpc_sketch_alloc<A>::check_lg_k(uint8_t lg_k) {
    if (lg_k < CPC_MIN_LG_K || lg_k > CPC_MAX_LG_K) {
        throw std::invalid_argument("lg_k must be >= " + std::to_string(CPC_MIN_LG_K) + " and <= " +
                                    std::to_string(CPC_MAX_LG_K) + ": " + std::to_string(lg_k));
    }
}
...
cpc_sketch_alloc<A>::cpc_sketch_alloc(uint8_t lg_k, uint32_t num_coupons,
                                      uint8_t first_interesting_column, u32_table<A>&& table,
                                      vector_u8<A>&& window, bool has_hip, double kxp,
...
template <typename A>
uint8_t cpc_sketch_alloc<A>::get_preamble_ints(uint32_t num_coupons, bool has_hip, bool has_table,
                                               bool has_window) {
...
template <typename A>
typename cpc_sketch_alloc<A>::flavor cpc_sketch_alloc<A>::determine_flavor(uint8_t lg_k,
                                                                           uint64_t c) {
    const uint32_t k = 1 << lg_k;
    ...
    if (c8 < 27 * k)
        return PINNED;  //   K/2 <= C < 27K/8
    else
        return SLIDING;  // 27K/8 <= C
}
...
vector_u64<A> cpc_sketch_alloc<A>::build_bit_matrix() const {
    ...
    if (sliding_window.size() > 0) {  // In other words, we are in window mode, not sparse mode
        for (size_t i = 0; i < k;
             i++) {  // set the window bits, trusting the sketch's current offset
...
template <typename A>
// NOLINTNEXTLINE(runtime/references)
void cpc_sketch_alloc<A>::write_hip(google::protobuf::io::CodedOutputStream& stream) const {
    // 将double转换为string，然后传入到pb中
    write_double(stream, kxp);
    write_double(stream, hip_est_accum);
}

template <typename A>
// NOLINTNEXTLINE(runtime/references)
void cpc_sketch_alloc<A>::write_double(google::protobuf::io::CodedOutputStream& stream,
                                       const double item) const {
    stream.WriteRaw(&item, sizeof(item));
}

template <typename A>
// NOLINTNEXTLINE(runtime/references)
double cpc_sketch_alloc<A>::read_double(google::protobuf::io::CodedInputStream& is) {
    double res = 0;
    is.ReadRaw(&res, sizeof(double));
    return res;
}

template <typename A>
int64_t cpc_sketch_alloc<A>::mem_size() const {
    // uint8_t(1) * 3 + uint32_t(4) + uint64_t(8) + double(8) * 2 + bool(1)
    return 32 + surprising_value_table.mem_size() + sliding_window.size();
}
