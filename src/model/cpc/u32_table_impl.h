...
u32_table<A>::u32_table(const A& allocator)
    : lg_size(0), num_valid_bits(0), num_items(0), slots(allocator) {}
...
u32_table<A>::u32_table(uint8_t lg_size, uint8_t num_valid_bits, const A& allocator)
    ...
    if (num_valid_bits < 1 || num_valid_bits > 32)
        throw std::invalid_argument("num_valid_bits must be between 1 and 32");
}
...
bool u32_table<A>::maybe_delete(uint32_t item) {
    ...
    // shrink if necessary
    if (U32_TABLE_DOWNSIZE_DENOM * num_items < U32_TABLE_DOWNSIZE_NUMER * (1 << lg_size) &&
        lg_size > 2) {
        rebuild(lg_size - 1);
    }
    return true;
...
template <typename A>
u32_table<A> u32_table<A>::make_from_pairs(const uint32_t* pairs, uint32_t num_pairs, uint8_t lg_k,
                                           const A& allocator) {
    uint8_t lg_num_slots = 2;
    while (U32_TABLE_UPSIZE_DENOM * num_pairs > U32_TABLE_UPSIZE_NUMER * (1 << lg_num_slots))
        lg_num_slots++;
    ...
    // pairs array However, we are starting out with the correct final table size, so the problem
    // might not occur
...
vector_u32<A> u32_table<A>::unwrapping_get_items() const {
    ...
    while (i < table_size && slots[i] != UINT32_MAX) {
        ...
        if (item & hi_bit) {
            result[r--] = item;
        } else {  // this item was probably wrapped, so move to end
            result[l++] = item;
        }
    }
...
template <typename A>
void u32_table<A>::merge(const uint32_t* arr_a, size_t start_a, size_t length_a,  // input
...
template <typename A>
int64_t u32_table<A>::mem_size() const {
    // uint8_t(1) + uint8_t(1) + uint32_t(4) + uint32_t(4) * slot_size
    return 2 + (slots.size() + 1) * 4;
}
