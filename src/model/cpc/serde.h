...
#include "model/cpc/memory_operations.h"
...
struct serde {
    ...
    void deserialize(std::istream& is, T* items,
...
struct serde<T, typename std::enable_if<std::is_arithmetic<T>::value>::type> {
    void serialize(std::ostream& os, const T* items, unsigned num) const {
        ...
        if (failure || !os.good()) {
            throw std::runtime_error("error writing to std::ostream with " + std::to_string(num) +
                                     " items");
        }
    }
    void deserialize(std::istream& is, T* items, unsigned num) const {
        ...
        if (failure || !is.good()) {
            throw std::runtime_error("error reading from std::istream with " + std::to_string(num) +
                                     " items");
        }
    }
...
struct serde<std::string> {
    ...
    size_t size_of_item(const std::string& item) const { return sizeof(uint32_t) + item.size(); }
    ...
    size_t deserialize(const void* ptr, size_t capacity, std::string* items, unsigned num) const {
        ...
        if (failure) {
            // clean up what we've already allocated
            for (unsigned j = 0; j < i; ++j) items[j].~basic_string();
