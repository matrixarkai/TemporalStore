...
#include "model/cpc/compression_data.h"
#include "model/cpc/count_zeros.h"
#include "model/cpc/cpc_common.h"
#include "model/cpc/cpc_util.h"
...
cpc_compressor<A>& get_compressor() {
    static cpc_compressor<A>* instance =
        new cpc_compressor<A>();  // use new for global initialization
    return *instance;
}
...
uint16_t* cpc_compressor<A>::make_decoding_table(const uint16_t* encoding_table,
    ...
    for (unsigned byte_value = 0; byte_value < num_byte_values; byte_value++) {
        ...
        for (uint32_t garbage_bits = 0; garbage_bits < num_copies; garbage_bits++) {
            const uint16_t extended_code_value =
                static_cast<uint16_t>(code_value | (garbage_bits << code_length));
...
template <typename A>
void cpc_compressor<A>::validate_decoding_table(const uint16_t* decoding_table,
                                                const uint16_t* encoding_table) const {
    for (int decode_this = 0; decode_this < 4096; decode_this++) {
        ...
        if (encoded_bit_pattern != (decode_this & ((1 << decoded_length) - 1)))
            throw std::logic_error("bit pattern error");
    }
}
...
void cpc_compressor<A>::make_decoding_tables() {
    ...
    validate_decoding_table(length_limited_unary_decoding_table65,
                            length_limited_unary_encoding_table65);

    for (int i = 0; i < (16 + 6); i++) {
        decoding_tables_for_high_entropy_byte[i] =
            make_decoding_table(encoding_tables_for_high_entropy_byte[i], 256);
        validate_decoding_table(decoding_tables_for_high_entropy_byte[i],
                                encoding_tables_for_high_entropy_byte[i]);
    }

    for (int i = 0; i < 16; i++) {
        column_permutations_for_decoding[i] =
            make_inverse_permutation(column_permutations_for_encoding[i], 56);
    }
}
...
template <typename A>
void cpc_compressor<A>::compress(const cpc_sketch_alloc<A>& source,
// NOLINTNEXTLINE(runtime/references)
    ...
    case cpc_sketch_alloc<A>::flavor::SLIDING:
        ...
        break;
    default:
        throw std::logic_error("Unknown sketch flavor");
    }
}

template <typename A>
// NOLINTNEXTLINE(runtime/references)
void cpc_compressor<A>::uncompress(const compressed_state<A>& source, uncompressed_state<A>& target,
    ...
    case cpc_sketch_alloc<A>::flavor::SLIDING:
        ...
        break;
    default:
        std::logic_error("Unknown sketch flavor");
    }
}

template <typename A>
void cpc_compressor<A>::compress_sparse_flavor(const cpc_sketch_alloc<A>& source,
// NOLINTNEXTLINE(runtime/references)
                                               compressed_state<A>& result) const {
...
template <typename A>
void cpc_compressor<A>::uncompress_sparse_flavor(const compressed_state<A>& source,
// NOLINTNEXTLINE(runtime/references)
                                                 uncompressed_state<A>& target,
                                                 uint8_t lg_k) const {
    ...
    vector_u32<A> pairs = uncompress_surprising_values(
        source.table_data.data(), source.table_data_words, source.table_num_entries, lg_k,
...
template <typename A>
void cpc_compressor<A>::compress_hybrid_flavor(const cpc_sketch_alloc<A>& source,
// NOLINTNEXTLINE(runtime/references)
                                               compressed_state<A>& result) const {
    ...
    const uint32_t num_pairs_from_window =
        source.get_num_coupons() - num_pairs_from_table;  // because the window offset is zero

    vector_u32<A> all_pairs =
        tricky_get_pairs_from_window(source.sliding_window.data(), k, num_pairs_from_window,
                                     num_pairs_from_table, source.get_allocator());

    u32_table<A>::merge(pairs_from_table.data(), 0, pairs_from_table.size(), all_pairs.data(),
                        num_pairs_from_table, num_pairs_from_window, all_pairs.data(),
...
template <typename A>
void cpc_compressor<A>::uncompress_hybrid_flavor(const compressed_state<A>& source,
// NOLINTNEXTLINE(runtime/references)
                                                 uncompressed_state<A>& target,
                                                 uint8_t lg_k) const {
    ...
    target.table =
        u32_table<A>::make_from_pairs(pairs.data(), next_true_pair, lg_k, pairs.get_allocator());
}

template <typename A>
void cpc_compressor<A>::compress_pinned_flavor(const cpc_sketch_alloc<A>& source,
// NOLINTNEXTLINE(runtime/references)
                                               compressed_state<A>& result) const {
...
template <typename A>
void cpc_compressor<A>::uncompress_pinned_flavor(const compressed_state<A>& source,
// NOLINTNEXTLINE(runtime/references)
                                                 uncompressed_state<A>& target, uint8_t lg_k,
                                                 uint32_t num_coupons) const {
    if (source.window_data.size() == 0) throw std::logic_error("window is expected");
    uncompress_sliding_window(source.window_data.data(), source.window_data_words, target.window,
                              lg_k, num_coupons);
    ...
    if (num_pairs == 0) {
        ...
        vector_u32<A> pairs =
            uncompress_surprising_values(source.table_data.data(), source.table_data_words,
                                         num_pairs, lg_k, source.table_data.get_allocator());
        ...
        target.table =
            u32_table<A>::make_from_pairs(pairs.data(), num_pairs, lg_k, pairs.get_allocator());
    }
}

template <typename A>
void cpc_compressor<A>::compress_sliding_flavor(const cpc_sketch_alloc<A>& source,
// NOLINTNEXTLINE(runtime/references)
                                                compressed_state<A>& result) const {
    compress_sliding_window(source.sliding_window.data(), source.get_lg_k(),
                            source.get_num_coupons(), result);
    ...
    if (pairs.size() > 0) {
        ...
        const uint8_t pseudo_phase =
            determine_pseudo_phase(source.get_lg_k(), source.get_num_coupons());
        if (pseudo_phase >= 16)
            throw std::logic_error("unexpected pseudo phase for sliding flavor");
        ...
        for (size_t i = 0; i < pairs.size(); i++) {
            ...
            // first rotate the columns into a canonical configuration: new = ((old - (offset+8)) +
            // 64) mod 64
...
template <typename A>
void cpc_compressor<A>::uncompress_sliding_flavor(const compressed_state<A>& source,
// NOLINTNEXTLINE(runtime/references)
                                                  uncompressed_state<A>& target, uint8_t lg_k,
                                                  uint32_t num_coupons) const {
    if (source.window_data.size() == 0) throw std::logic_error("window is expected");
...
template <typename A>
void cpc_compressor<A>::compress_surprising_values(const vector_u32<A>& pairs, uint8_t lg_k,
// NOLINTNEXTLINE(runtime/references)
                                                   compressed_state<A>& result) const {
    const uint32_t k = 1 << lg_k;
    ...
    uint32_t csv_length = low_level_compress_pairs(
        pairs.data(), static_cast<uint32_t>(pairs.size()), num_base_bits, result.table_data.data());
...
vector_u32<A> cpc_compressor<A>::uncompress_surprising_values(const uint32_t* data,
                                                              uint32_t data_words,
                                                              uint32_t num_pairs, uint8_t lg_k,
...
template <typename A>
void cpc_compressor<A>::compress_sliding_window(const uint8_t* window, uint8_t lg_k,
                                                uint32_t num_coupons,
                                                // NOLINTNEXTLINE(runtime/references)
                                                compressed_state<A>& target) const {
    const uint32_t k = 1 << lg_k;
    const size_t window_buf_len = safe_length_for_compressed_window_buf(k);
    target.window_data.resize(window_buf_len);
    const uint8_t pseudo_phase = determine_pseudo_phase(lg_k, num_coupons);
    size_t data_words = low_level_compress_bytes(
        window, k, encoding_tables_for_high_entropy_byte[pseudo_phase], target.window_data.data());
...
// NOLINTNEXTLINE(runtime/references)
                                                  vector_u8<A>& window, uint8_t lg_k,
                                                  uint32_t num_coupons) const {
    const uint32_t k = 1 << lg_k;
    window.resize(k);  // zeroing not needed here (unlike the Hybrid Flavor)
    const uint8_t pseudo_phase = determine_pseudo_phase(lg_k, num_coupons);
    low_level_uncompress_bytes(
        window.data(), k, decoding_tables_for_high_entropy_byte[pseudo_phase], data, data_words);
}

template <typename A>
size_t cpc_compressor<A>::safe_length_for_compressed_pair_buf(uint32_t k, uint32_t num_pairs,
...
size_t cpc_compressor<A>::safe_length_for_compressed_window_buf(
    uint32_t k) {  // measured in 32-bit words
    const size_t bits =
        12 * k + 11;  // 11 bits of padding, due to 12-bit lookahead, with 1 bit certainly present.
...
uint8_t cpc_compressor<A>::determine_pseudo_phase(uint8_t lg_k, uint32_t c) {
    ...
    if (1000 * c < 2375 * k) {
        if (4 * c < 3 * k)
            return 16 + 0;  // mid-range table
        else if (10 * c < 11 * k)
            return 16 + 1;  // mid-range table
        else if (100 * c < 132 * k)
            return 16 + 2;  // mid-range table
        else if (3 * c < 5 * k)
            return 16 + 3;  // mid-range table
        else if (1000 * c < 1965 * k)
            return 16 + 4;  // mid-range table
        else if (1000 * c < 2275 * k)
            return 16 + 5;  // mid-range table
        else
...
static inline void maybe_flush_bitbuf(uint64_t& bitbuf, uint8_t& bufbits, uint32_t* wordarr,
                                      uint32_t& wordindex) {  // NOLINT(runtime/references)
    if (bufbits >= 32) {
...
// NOLINTNEXTLINE(runtime/references)
                                     uint32_t& wordindex, uint8_t minbits) {
    if (bufbits < minbits) {
...
template <typename A>
uint32_t cpc_compressor<A>::low_level_compress_bytes(const uint8_t* byte_array,  // input
    ...
    // Pad the bitstream with 11 zero-bits so that the decompressor's 12-bit peek can't overrun its
    // input.
    bufbits += 11;
    maybe_flush_bitbuf(bitbuf, bufbits, compressed_words, next_word_index);
...
template <typename A>
void cpc_compressor<A>::low_level_uncompress_bytes(uint8_t* byte_array,  // output
    ...
    for (uint32_t byte_index = 0; byte_index < num_bytes_to_decode; byte_index++) {
        maybe_fill_bitbuf(bitbuf, bufbits, compressed_words, word_index,
                          12);  // ensure 12 bits in bit buffer

        const size_t peek12 =
            bitbuf & 0xfff;  // These 12 bits will include an entire Huffman codeword.
    ...
    if (word_index > num_compressed_words)
        throw std::logic_error("word_index > num_compressed_words");
}

// NOLINTNEXTLINE(runtime/references)
static inline uint64_t read_unary(const uint32_t* compressed_words, uint32_t& next_word_index,
// NOLINTNEXTLINE(runtime/references)
                                  uint64_t& bitbuf, uint8_t& bufbits);

// NOLINTNEXTLINE(runtime/references)
static inline void write_unary(uint32_t* compressed_words, uint32_t& next_word_index_ptr,
// NOLINTNEXTLINE(runtime/references)
                               uint64_t& bit_buf_ptr, uint8_t& buf_bits_ptr, uint64_t value);
...
template <typename A>
uint32_t cpc_compressor<A>::low_level_compress_pairs(const uint32_t* pair_array,  // input
    ...
    for (uint32_t pair_index = 0; pair_index < num_pairs_to_encode; pair_index++) {
        ...
        if (row_index < predicted_row_index)
            throw std::logic_error("row_index < predicted_row_index");
        if (col_index < predicted_col_index)
            throw std::logic_error("col_index < predicted_col_index");
...
template <typename A>
void cpc_compressor<A>::low_level_uncompress_pairs(uint32_t* pair_array,  // output
    ...
    for (uint32_t pair_index = 0; pair_index < num_pairs_to_decode; pair_index++) {
        ...
        maybe_fill_bitbuf(bitbuf, bufbits, compressed_words, word_index,
                          num_base_bits);  // ensure num_base_bits in bit buffer
    ...
    if (word_index > num_compressed_words)
        throw std::logic_error("word_index > num_compressed_words");  // check for buffer over-run
}

// NOLINTNEXTLINE(runtime/references)
uint64_t read_unary(const uint32_t* compressed_words, uint32_t& next_word_index, uint64_t& bitbuf,
                    uint8_t& bufbits) {  // NOLINT(runtime/references)
    if (compressed_words == nullptr) throw std::logic_error("compressed_words == NULL");
    size_t subtotal = 0;
    while (true) {
        maybe_fill_bitbuf(bitbuf, bufbits, compressed_words, next_word_index,
                          8);  // ensure 8 bits in bit buffer

        const uint8_t peek8 =
            bitbuf & 0xff;  // These 8 bits include either all or part of the Unary codeword
...
// NOLINTNEXTLINE(runtime/references)
void write_unary(uint32_t* compressed_words, uint32_t& next_word_index, uint64_t& bitbuf,
                 uint8_t& bufbits, uint64_t value) {  // NOLINT(runtime/references)
    if (compressed_words == nullptr) throw std::logic_error("compressed_words == NULL");
...
template <typename A>
vector_u32<A> cpc_compressor<A>::tricky_get_pairs_from_window(const uint8_t* window, uint32_t k,
                                                              uint32_t num_pairs_to_get,
...
uint8_t cpc_compressor<A>::golomb_choose_number_of_base_bits(uint32_t k, uint64_t count) {
    ...
    if (quotient == 0)
        ...
        return floor_log2_of_long(quotient);
}
