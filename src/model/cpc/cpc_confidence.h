...
//  1,    2,    3, // kappa
static const int16_t ICON_LOW_SIDE_DATA[33] = {
    // Empirically measured at N = 1000 * K.
...
//  1,    2,    3, // kappa
static const int16_t ICON_HIGH_SIDE_DATA[33] = {
    // Empirically measured at N = 1000 * K.
...
//  1,    2,    3, // kappa
static const int16_t HIP_LOW_SIDE_DATA[33] = {
    // Empirically measured at N = 1000 * K.
...
//  1,    2,    3, // kappa
static const int16_t HIP_HIGH_SIDE_DATA[33] = {
    // Empirically measured at N = 1000 * K.
...
double get_icon_confidence_lb(const cpc_sketch_alloc<A>& sketch, int kappa) {
    ...
    if (lg_k <= 14) {
        x = (static_cast<double>(ICON_HIGH_SIDE_DATA[3 * (lg_k - 4) + (kappa - 1)])) / 10000.0;
    }
...
double get_icon_confidence_ub(const cpc_sketch_alloc<A>& sketch, int kappa) {
    ...
    double x = ICON_ERROT_CONSTANT;
    if (lg_k <= 14) {
        x = (static_cast<double>(ICON_LOW_SIDE_DATA[3 * (lg_k - 4) + (kappa - 1)])) / 10000.0;
    }
    const double rel = x / sqrt(k);
    const double eps = kappa * rel;
    const double est = sketch.get_icon_estimate();
...
double get_hip_confidence_lb(const cpc_sketch_alloc<A>& sketch, int kappa) {
    ...
    if (lg_k <= 14) {
        x = (static_cast<double>(HIP_HIGH_SIDE_DATA[3 * (lg_k - 4) + (kappa - 1)])) / 10000.0;
    }
    const double rel = x / (sqrt(static_cast<double>(k)));
    const double eps = (static_cast<double>(kappa)) * rel;
    const double est = sketch.get_hip_estimate();
    double result = est / (1.0 + eps);
    const double check = static_cast<double>(sketch.get_num_coupons());
    if (result < check) result = check;
    return result;
}
...
double get_hip_confidence_ub(const cpc_sketch_alloc<A>& sketch, int kappa) {
    ...
    double x = HIP_ERROR_CONSTANT;
    if (lg_k <= 14) {
        x = (static_cast<double>(HIP_LOW_SIDE_DATA[3 * (lg_k - 4) + (kappa - 1)])) / 10000.0;
    }
    const double rel = x / sqrt(k);
    const double eps = kappa * rel;
    const double est = sketch.get_hip_estimate();
