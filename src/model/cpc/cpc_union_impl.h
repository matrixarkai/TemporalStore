...
cpc_union_alloc<A>::cpc_union_alloc(uint8_t lg_k, uint64_t seed, const A& allocator)
    : lg_k(lg_k), seed(seed), accumulator(nullptr), bit_matrix(allocator) {
...
cpc_union_alloc<A>::cpc_union_alloc(const cpc_union_alloc<A>& other)
    ...
    if (accumulator != nullptr) {
        accumulator = new (AllocCpc(accumulator->get_allocator()).allocate(1))
...
void cpc_union_alloc<A>::internal_update(S&& sketch) {
    ...
    if (accumulator == nullptr && bit_matrix.size() == 0)
        throw std::logic_error("both accumulator and bit matrix are absent");
    ...
    if (cpc_sketch_alloc<A>::flavor::SPARSE == src_flavor && accumulator != nullptr) {  // Case A
        ...
        if (final_dst_flavor != cpc_sketch_alloc<A>::flavor::EMPTY &&
            final_dst_flavor != cpc_sketch_alloc<A>::flavor::SPARSE) {
    ...
    if (cpc_sketch_alloc<A>::flavor::HYBRID != src_flavor &&
        cpc_sketch_alloc<A>::flavor::PINNED != src_flavor &&
        cpc_sketch_alloc<A>::flavor::SLIDING != src_flavor)
        throw std::logic_error("wrong flavor");
    ...
    if (accumulator != nullptr) {
        ...
        if (cpc_sketch_alloc<A>::flavor::EMPTY != dst_flavor &&
            cpc_sketch_alloc<A>::flavor::SPARSE != dst_flavor) {
            throw std::logic_error("wrong flavor");
        }
        switch_to_bit_matrix();
    }
    ...
    if (cpc_sketch_alloc<A>::flavor::HYBRID == src_flavor ||
        cpc_sketch_alloc<A>::flavor::PINNED == src_flavor) {  // Case C
    ...
    if (cpc_sketch_alloc<A>::flavor::SLIDING != src_flavor)
        throw std::logic_error("wrong flavor");  // Case D
...
cpc_sketch_alloc<A> cpc_union_alloc<A>::get_result_from_accumulator() const {
    ...
    if (accumulator->determine_flavor() != cpc_sketch_alloc<A>::flavor::SPARSE)
        throw std::logic_error("wrong flavor");
...
cpc_sketch_alloc<A> cpc_union_alloc<A>::get_result_from_bit_matrix() const {
    ...
    if (flavor != cpc_sketch_alloc<A>::flavor::HYBRID &&
        flavor != cpc_sketch_alloc<A>::flavor::PINNED &&
        flavor != cpc_sketch_alloc<A>::flavor::SLIDING)
        throw std::logic_error("wrong flavor");
    ...
    return cpc_sketch_alloc<A>(lg_k, num_coupons, first_interesting_column, std::move(table),
                               std::move(sliding_window), false, 0, 0, seed);
}
...
void cpc_union_alloc<A>::walk_table_updating_sketch(const u32_table<A>& table) {
    ...
    const uint64_t dst_mask =
        (((1 << accumulator->get_lg_k()) - 1) << 6) | 63;  // downsamples when dst lgK < src LgK
...
template <typename A>
void cpc_union_alloc<A>::or_window_into_matrix(const vector_u8<A>& sliding_window, uint8_t offset,
                                               uint8_t src_lg_k) {
...
void cpc_union_alloc<A>::reduce_k(uint8_t new_lg_k) {
    ...
    if (accumulator == nullptr && bit_matrix.size() == 0)
        throw std::logic_error("both accumulator and bit_matrix are absent");
