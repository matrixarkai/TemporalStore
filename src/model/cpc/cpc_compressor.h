...
// the sketch implementation uses this global function to statically allocate and construct on the
// first use
template <typename A>
...
class cpc_compressor {
 public:
    ...
    uint32_t low_level_compress_bytes(const uint8_t* byte_array,  // input
                                      uint32_t num_bytes_to_encode, const uint16_t* encoding_table,
                                      uint32_t* compressed_words) const;  // output

    void low_level_uncompress_bytes(uint8_t* byte_array,  // output
                                    uint32_t num_bytes_to_decode, const uint16_t* decoding_table,
                                    const uint32_t* compressed_words,
                                    uint32_t num_compressed_words) const;  // input
    ...
    uint32_t low_level_compress_pairs(const uint32_t* pair_array,  // input
                                      uint32_t num_pairs_to_encode, uint8_t num_base_bits,
                                      uint32_t* compressed_words) const;  // output

    void low_level_uncompress_pairs(uint32_t* pair_array,  // output
                                    uint32_t num_pairs_to_decode, uint8_t num_base_bits,
 ...
 private:
    ...
    uint8_t* column_permutations_for_decoding[16] = {NULL, NULL, NULL, NULL, NULL, NULL,
                                                     NULL, NULL, NULL, NULL, NULL, NULL,
                                                     NULL, NULL, NULL, NULL};
    ...
    void compress_sparse_flavor(const cpc_sketch_alloc<A>& source,
    // NOLINTNEXTLINE(runtime/references)
                                compressed_state<A>& target) const;
    void compress_hybrid_flavor(const cpc_sketch_alloc<A>& source,
    // NOLINTNEXTLINE(runtime/references)
                                compressed_state<A>& target) const;
    void compress_pinned_flavor(const cpc_sketch_alloc<A>& source,
    // NOLINTNEXTLINE(runtime/references)
                                compressed_state<A>& target) const;
    void compress_sliding_flavor(const cpc_sketch_alloc<A>& source,
    // NOLINTNEXTLINE(runtime/references)
                                 compressed_state<A>& target) const;

    // NOLINTNEXTLINE(runtime/references)
    void uncompress_sparse_flavor(const compressed_state<A>& source, uncompressed_state<A>& target,
                                  uint8_t lg_k) const;
    // NOLINTNEXTLINE(runtime/references)
    void uncompress_hybrid_flavor(const compressed_state<A>& source, uncompressed_state<A>& target,
                                  uint8_t lg_k) const;
    // NOLINTNEXTLINE(runtime/references)
    void uncompress_pinned_flavor(const compressed_state<A>& source, uncompressed_state<A>& target,
                                  uint8_t lg_k, uint32_t num_coupons) const;
    // NOLINTNEXTLINE(runtime/references)
    void uncompress_sliding_flavor(const compressed_state<A>& source, uncompressed_state<A>& target,
                                   uint8_t lg_k, uint32_t num_coupons) const;
    ...
    void validate_decoding_table(const uint16_t* decoding_table,
    ...
    void compress_sliding_window(const uint8_t* window, uint8_t lg_k, uint32_t num_coupons,
                                 compressed_state<A>& target) const;  // NOLINT(runtime/references)

    vector_u32<A> uncompress_surprising_values(const uint32_t* data, uint32_t data_words,
                                               uint32_t num_pairs, uint8_t lg_k,
                                               const A& allocator) const;
    // NOLINTNEXTLINE(runtime/references)
    void uncompress_sliding_window(const uint32_t* data, uint32_t data_words, vector_u8<A>& window,
                                   uint8_t lg_k, uint32_t num_coupons) const;

    static size_t safe_length_for_compressed_pair_buf(uint32_t k, uint32_t num_pairs,
    ...
    static inline vector_u32<A> tricky_get_pairs_from_window(const uint8_t* window, uint32_t k,
...
#include "model/cpc/cpc_compressor_impl.h"
