...
1. The engine now supports version-gated shard reload: missing shards still load normally, loaded
   shards reject stale load versions, and accepted reloads update table/URI/slot/readonly metadata
   without dropping membership state.
2. The data-node runtime now exposes a lifecycle report with loaded, serving, readonly, queued,
   running, unloading, failed, max-load-version, and per-shard serving-state accounting.
3. Preflight now embeds the lifecycle report so operators can see node serving state alongside
   metaserver heartbeat, topology validation, queue, dirty-object, and runtime counters.
4. Server routes now include direct Rust admin endpoints `POST /reload` and `GET /server/lifecycle`
   plus C++-style aliases `POST /ServerService/Reload` and `GET /ServerService/GetLifecycle`.
5. Regression coverage validates stale reload rejection, metadata update, membership preservation,
...
20. Closed the shared-store checkpoint-retention gap: Rust now has cursor-safe
    checkpoint GC that keeps the newest requested checkpoints plus the latest
...
classes, layout transition counters, object-page ownership transition counters,
...
14. Closed storage visibility gap: Rust recovery, recovery-boundary, and
    ...
    object-id conflicts, missing owner refs, and owner mismatches. This improves
    C++ `ObjectManager`/page ownership parity without changing the Rust page
    envelope format.
15. Closed a slot dump/load lifecycle gap: new Rust slot dump manifests are v3
    and carry selected-slot object lifecycle metadata. Manifest validation now
    rejects lifecycle/index mismatches even if a tampered manifest recomputes its
    checksum and generation id, tightening C++-style merged dump/load safety for
    object/page/slot ownership.
16. Closed a node heartbeat/load-report gap: Rust data nodes now build
    C++-style runtime load and per-shard serving-state reports, the server
...
18. Tightened the Raft runtime parity gate: `OpenRaftDataNodeProcessRolloutReport`
    ...
    leader transfer exact-once evidence, leader transfer under load, snapshot bootstrap,
    snapshot install/restart, membership rescale, membership
...
| Hot object model | `ObjectManager`, model objects, dirty slots | per-type maps of key/field/timestamp to `BlockAddress` plus stable page/object/routing-slot metadata, persisted first-class `CoreIndex -> SlotMap -> SlotNode -> PageIndex/ObjectIndex` ownership index updated incrementally on changed objects and used as the authoritative existence/read path, legacy model-map-only state promoted into the slot index on load/execute, recovery reconciliation from slot index back to secondary model views, cold reads through `PageIndex -> PageAddress -> cache/page-store`, slot dirty generations, SlotNode TTL/deleted/loading/in-memory flags, PageIndex-like model id/page id/dirty/deleted/log/size/address metadata, ownership report separating core index authority from legacy promotion, per-slot storage summaries for object/page refs, logical/physical bytes, dirty generation/counts, page segments and last zone, C++-style stats for logical object count, page refs, dirty objects, dirty routing slots, partition info, and recovery/lifecycle object counters for live object IDs, hot/cold/mixed/deleted residency, hot/cold/deleted page refs, object-page ownership transitions, stale page estimates, tombstones, reused object-id conflicts, missing owner refs, and owner mismatches | full byte-for-byte C++ ObjectManager hot-object memory layout, full model-specific memory layout |
