...
| Local storage | Local page segments + JSON index, oplog/index-log, periodic dirty-shard dump scheduling, local live-page rewrite compaction, C++-ordered StorageManager cycle, and Rust-native merged dump/load policy evidence for manifest validation, sequence boundaries, install preflight, reclaim, compaction, index-GC, and cache policy | Oplog + page/slot store + dump/recover | Need binary log/header parity, deeper C++ object/page/slot layout, production cleaner weights, and byte-for-byte C++ zone/page-header compaction parity. |
