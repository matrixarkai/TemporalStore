...
| Local storage | Local page segments + JSON index, oplog/index-log, periodic dirty-shard dump scheduling, and local live-page rewrite compaction | Oplog + page/slot store + dump/recover | Need binary log/header parity, C++ dirty-slot ownership, recovery boundaries, merged dump/load, and zone/page-header compaction parity. |
