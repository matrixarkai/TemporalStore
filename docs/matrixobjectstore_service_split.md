...
Append-only shared-store objects such as WAL/oplog entries, snapshot temp uploads, and final stable snapshot objects use `ObjectStore::put_unique`, which skips the previous-manifest lookup and stale-ref cleanup that normal overwrite-capable writes need.
