...
The manifest records the source prefix, target prefix, mode, source range, partial filters, checkpoint key, and final summary. It also includes `manifest_schema="matrixark_context_backfill_manifest_v1"` and `manifest_payload_sha256`, a SHA-256 digest over the canonical manifest payload before the digest field is added. The run summary repeats the same schema and digest so archived JSON summaries can be matched back to the persisted target manifest. This is the main audit artifact for production recovery.

Use `verify_manifest` to re-check a persisted target manifest during release review or incident handoff:

```bash
python3 tools/matrixark_context_backfill.py \
  --mode=verify_manifest \
  --local-kv=/var/lib/matrixark/backfill/local_kv.json \
  --target-prefix=matrixark:context_backfill:<job_id> \
  --job-id=<job_id> \
  --raw-backend=temporalstore \
  --prometheus-output=/var/tmp/matrixark_backfill_verify_manifest.prom
```

The command returns `status="ok"` only when the manifest exists, uses the supported schema, matches the requested job and target prefix, matches the selected raw backend, and recomputes the same `manifest_payload_sha256`. A failed `manifest_payload_sha256_match` means the manifest or its embedded summary changed after the run and should not be accepted as release or recovery evidence.

When `--prometheus-output` is set, `verify_manifest` emits `matrixark_context_backfill_manifest_verification_status` and `matrixark_context_backfill_manifest_verification_check`. Scrape or archive this file with the JSON result so release review and incident handoff can alert on missing, invalid, mismatched, or tampered manifests without parsing CLI stdout.
...
Keep this gate green before changing backfill defaults, raw-message storage behavior, validation semantics, or benchmark thresholds. CI systems should call this script directly and upload `matrixark_context_backfill_readiness.json` plus `matrixark_context_backfill_evidence/` as artifacts. Set `MATRIXARK_BACKFILL_CI_EVIDENCE_DIR=<path>` when the CI system needs a different artifact directory, and set `MATRIXARK_BACKFILL_CI_EVIDENCE_MANIFEST=<path>` only when the top-level manifest must live outside that directory. The top-level manifest stores artifact paths relative to the manifest directory, so the uploaded evidence bundle remains verifiable after download or relocation. The script verifies `matrixark_context_backfill_evidence/manifest.json` with `--require-relative-paths=1` before reporting success and writes `matrixark_context_backfill_evidence/manifest.prom` with `matrixark_context_backfill_ci_evidence_verification_status` plus per-check metrics. The evidence verifier validates checksums and also checks that the readiness report has `status="ok"`, all readiness checks passed, all required readiness gate sections report `status="ok"`, and the dual-write readiness JSON reports `status="ok"`. It also verifies that the dual-write evidence manifest uses `matrixark_dual_write_readiness_evidence_v1`, the dual-write evidence manifest reports `status="ok"`, and its required artifact entries are present. The dual-write evidence manifest stores portable relative artifact paths, and the verifier confirms that dual-write evidence manifest artifact paths stay inside the dual-write evidence directory and that dual-write evidence manifest artifact sizes and SHA-256 checksums match. When nested evidence fails, `nested_verified_artifacts` identifies the nested artifact, stored path, containment result, size match, and SHA-256 match so release reviewers can diagnose bad uploads quickly. Release reviewers can re-run `python3 tools/verify_matrixark_context_backfill_ci_evidence.py --manifest matrixark_context_backfill_evidence/manifest.json --require-relative-paths=1 --prometheus-output matrixark_context_backfill_evidence/manifest.prom` before accepting uploaded evidence, then inspect both `matrixark_context_backfill_evidence/manifest.json` and `matrixark_context_backfill_evidence/dual_write/manifest.json`.
...
| `batch_size_summary` | Per-batch-size QPS/latency summaries plus recommended batch sizes for full shadow, incremental repair, partial repair, and balanced throughput. It also includes `production_candidate`, the safest first batch-size candidate for production because it maximizes the minimum QPS across full shadow, incremental repair, and partial repair while preserving p95 latency and raw-backend parity evidence. |
...
The `batch_size_summary.production_candidate` field is the safest first pick when full rebuilds and incremental repairs both matter. It is derived from `best_balanced_min_qps` and includes phase QPS, p95 phase latency, and raw-backend parity ratio in one object. Prefer the repair-specific recommendation only for incident-recovery profiles where bounded active-prefix repair latency matters more than full rebuild throughput.
...
Activation and repair audit records include both the nested validation response and flattened `validation_status`, `validation_skipped`, `validation_skip_reason`, `validation_source_range`, and `validation_target_state` fields so operators can query the cutover boundary and target evidence without parsing the full validation object. Skipping validation is a break-glass path: `activate_shadow` and `incremental_repair` reject `--skip-validation=1` unless `--confirm-skip-validation=YES` is also supplied. Skipped validation still inspects the target prefix for both activation and incremental repair; an empty target, unreadable record, missing record, or dead letter is rejected unless `--confirm-unvalidated-target-state=YES` is supplied. That override is only for emergency cutovers or repair promotions and is audited with `unvalidated_target_state_confirmed=true` plus the inspected `validation_target_state`. Non-strict validation is also explicit: those promotion paths reject `--validation-strict=0` unless `--confirm-non-strict-validation=YES` is supplied. If the validation bypass is explicitly confirmed, the audit records `validation_status=skipped`, `validation_skipped=true`, and `validation_skip_reason=skip_validation_flag`.
