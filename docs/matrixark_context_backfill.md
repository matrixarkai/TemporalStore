...
Validated activation also rejects an empty source or empty target by default. Use `--confirm-empty-activation=YES` only when an empty cutover is the reviewed intent, such as retiring a namespace or explicitly activating an empty test tenant; the activation audit records `empty_activation_confirmed=true`.
...
`chunk_plan.execution_plan` groups independent shadow and validation commands into waves of up to `--plan-parallelism` windows. Treat those waves as the fast preparation phase. When `--local-kv` is used for local file-backed testing, the planner automatically serializes waves with `plan_parallelism=1` and records the requested value under `requested_plan_parallelism`; the local JSON backend is intentionally not a multi-process write target. Real TemporalStore or MatrixKV backends keep the requested parallelism. The same plan also emits `promotion_sequence`, which should be run one command at a time in sequence because it updates the active serving prefix.
...
When `--plan-output-dir` is set, the planner writes `plan.json`, `artifact_manifest.json`, `shadow_wave_*.sh`, `validate_wave_*.sh`, and `promote_serial.sh`. The wave scripts run independent chunk work in parallel inside each wave. The promotion script runs repairs sequentially. Each generated script creates `execution_evidence/` beside the restored plan bundle and writes per-command JSON stdout, stderr logs, and Prometheus text output such as `shadow_wave_0000_cmd_0000.json`, `shadow_wave_0000_cmd_0000.stderr.log`, and `shadow_wave_0000_cmd_0000.prom`. The manifest records bundle-relative paths, original paths, SHA-256, size, and executable bit for each generated file. The planner refuses to write into a non-empty output directory unless `--confirm-plan-output-overwrite=YES` is supplied, which prevents accidentally mixing evidence from different recovery runs. Review `plan.json` and the generated scripts before execution, then archive the whole directory, including `execution_evidence/`, with the recovery ticket or release evidence.
...
Validated `activate_shadow` additionally rejects cutovers where validation reports `expected_records == 0` or `actual_records == 0`. Use `--confirm-empty-activation=YES` only for an intentional empty activation; the activation response and audit record include `empty_activation_confirmed=true`.
...
- Use `--confirm-empty-activation=YES` only when a validated empty shadow activation is intentionally reviewed and should be audited.
