...
- Claude Code-style hooks: `tools/matrixark_agent_hook.py --agent claude` normalizes prompt, tool, resource, feedback, stop, and compact events.
- OpenClaw/OpenCode-style agents: `tools/matrixark_agent_config.py --client openclaw` emits MCP plus hook config, and `tools/matrixark_agent_hook.py --agent openclaw` handles lifecycle payloads.
- Cursor, Windsurf, Cline/Roo, Continue, Aider, Gemini/Qwen Code, AutoGen, LangGraph, CrewAI, LlamaIndex, Semantic Kernel, Dify, n8n, and generic agents use the same MCP server plus universal hook adapter.
- Regression coverage exists for Claude prompt ingest/retrieve, OpenClaw resource import, and OpenClaw session commit in `tools/test_matrixark_popular_agent_hooks.py`.
