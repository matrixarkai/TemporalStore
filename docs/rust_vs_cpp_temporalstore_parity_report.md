...
| Ingest/extract handoff counts | C++ debug flow reports the materialized node/event/index/dirty/summary records before retrieval | Implemented in Rust `ContextIngestExtractSummary` with extracted node, event, index-ref, dirty-marker, and summary-ref counts; covered by `context_management_ingest_retrieve_pipeline`. |
...
2. Convert C++ static parity gates into executable shared cases family by family:
   storage/Raft, control plane, ingestion, Context, Redis/admin, Feature, and Control State (formerly Risk).
