...
The production gate also writes `redis-production-benchmark-rollup.json`, a top-level rollup across repeated live benchmark runs, including the enforced `min_overall_qps_threshold`. It also writes `redis-production-gate-summary.json`, which ties the manifest hash, open-source surface validator, MatrixObject naming guard, and optional benchmark rollup into one top-level evidence artifact. The canonical manifest declares `surface: trimmed_open_source_context_feature_frequency` and the required/opt-in benchmark command sets. The benchmark summary copies that exact identity, expected/executed command coverage, and expected command count so perf artifacts cannot be confused with private/full Redis compatibility experiments.
