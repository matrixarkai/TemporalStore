...
The canonical manifest declares `surface: trimmed_open_source_context_feature_frequency` and the required/opt-in benchmark command sets. The benchmark summary copies that exact identity, command coverage, and expected command count so perf artifacts cannot be confused with private/full Redis compatibility experiments.
