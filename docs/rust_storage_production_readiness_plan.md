...
- `compat/storage_migration_corpus.json` defines C++-exported logical storage artifacts for common,
  string, hash, set, Feature, Sequence, Control State (formerly Risk), Redis-compatible, and Context model coverage.
...
covers string/hash/set/timestamped pages, Control State pages, and Context event/embedding/summary sidecar
...
| WAL/oplog structure, API, and flush parity | Rust WAL records expose versioned C++ OpLogger-style item metadata, idempotent replay append, explicit flush reports, persistent length/current sequence info, and legacy oplog filename compatibility. Rust still uses Rust-native JSONL WAL bytes, not C++ protobuf stream bytes. |
...
| Control State/context page-backed parity | Control State and context model tests must verify page-backed storage, secondary view reconciliation from slot/object/page authority, and logical reads after reload/compaction. |
...
real readiness evidence, but they are not treated as byte-for-byte C++ runtime parity. Exact
