...
- Add cold archive from TemporalStore to MatrixKV as the default cold/offline store, with MatrixDB as an optional high-QPS analytics sink.
  ...
  - If raw agent ingestion messages are stored in TemporalStore instead of MatrixKV/MatrixDB, treat them as a cold durable log, not hot context:
    ...
    - do not materialize raw message bodies in process-local hot caches or ContextPack-serving candidate caches; cache only tiny watermarks/cursors/checksums needed for idempotency and backfill progress;
    - provide a native C++/Rust backfill scanner that reads this cold TemporalStore log by tenant/user/session/time range in bounded batches and emits normalized context records through `matrixark_batch_append_records`;
    - expose metrics for cold raw-log bytes written, cold raw-log scans, no-promote reads, cache-admission skips, backfill lag, and raw-log retention/GC eligibility.
  ...
  - Temporal compression GC gap: temporal compression safety metadata exists today, but actual physical raw-event GC is not fully productionized. Add native C++ and Rust `matrixark_gc_expired_context_events` workers that use timestamp-keyed context-event indexes, no-cache/no-promote cold scans, bounded batches, archive/compression safety checks, and explicit tombstone plus audit records. The worker must not warm cold raw-event pages into the serving cache unless replay or user retrieval explicitly reinforces them. Temporal compression scans over old TemporalStore data must use the same cold/no-promote path as raw-log backfill scans: separate bounded buffers, lower priority, no hot-cache admission, no LRU recency update, and separate metrics from serving retrieval scans.
...
- Add future cross-user context sharing with explicit consent and policy gates.
  ...
  - Retrieval workflow:
    ...
    - context token budgets are intentionally split: `MATRIXARK_DEFAULT_MAX_CONTEXT_TOKENS` is the large total local+remote pack ceiling (default 128000), while verticals are bounded by both absolute caps and percentage caps: cross-session defaults to 20% max / 8192 tokens, shared resources to 25% max / 16384 tokens, and shared skills to 10% max / 8192 tokens.
