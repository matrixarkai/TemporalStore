...
| `storage_manager_metrics_admin_phase_reports` | Rust verifies the reusable StorageManager phase executor populates per-phase admin reports for last run time, duration, selected slots, skipped reason, errors, reclaimed bytes, compacted pages, WAL/index-log floors, retention blockers, and pressure before/after. |
