...
| `raft_openraft_process_rollout_evidence` | Production-readiness evidence case requiring LocalModel rejection plus OpenRaft spawned-process counts, independent WAL/snapshot dirs, observed requests, read-index responses, restart recovery, per-node log-store inspection, and ByteRaft-derived process-semantics evidence for peer pipeline state, snapshots, WAL lifecycle, failover, membership, and secondary lag where applicable. |
...
| `raft_byteraft_membership_roles` | ByteRaft-derived witness, learner, learner auto-promote, local-status role evidence, and pending joint-consensus restart behavior. |
