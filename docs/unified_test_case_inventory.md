...
| `storage_manager_real_pressure_signals` | Rust verifies StorageManager cycle pressure is driven by actual dirty slots, WAL/index-log bytes, stale page density, cache pressure, expiry scan debt, delayed-destroy backlog, follower retention blockers, and model-layout compaction debt. |
...
| `storage_manager_metrics_admin_phase_reports` | Rust verifies per-phase StorageManager admin reports expose last run time, duration, selected slots, skipped reason, errors, reclaimed bytes, compacted pages, WAL/index-log floors, retention blockers, and pressure before/after. |
