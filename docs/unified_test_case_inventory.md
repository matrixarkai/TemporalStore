...
| `raft_rustraft_metrics_admin_pipeline_status` | RustRaft-derived status/local-status/Prometheus peer pipeline, quorum peer progress, non-vacuous runtime pipeline activity, reorder queue depth/timeouts/dropped packages, apply health, read-index, and leader-transfer evidence. |
...
| `raft_byteraft_leader_election_learner_promotion_parity` | ByteRaft-style leader-election parity case requiring no-quorum pre-vote rejection, healed-quorum leader election, learner add/catch-up/promote, learner auto-promote, and exact-once leader-transfer commit evidence. |
...
| `control_client_retry_budget_topology_refresh` | Rust-executable/C++-static gate for client retry budgets, topology refresh, stale route invalidation, direct SDK parity evidence, and no duplicate unsafe writes. |
