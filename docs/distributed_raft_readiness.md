...
- ByteRaft-style process-path admin evidence through `ByteRaftRuntimeAdminReport`: per-peer
  ...
  append queue depth/max depth, active in-flight replicate limits, active apply batch
  backpressure, memory replicate-byte backpressure, oversized-log rejection, observed pressure
  counters, out-of-order append handling, append/reorder queue depth, reorder
