...
RustRaft-style reorder queue timeout/drop metrics in addition to oversized-log,
...
remain active. The RustRaft admin report exposes these as
...
- `tools/run_raft_distributed_conformance.sh`: combined data-node plus metaserver JSON validation
  passed.
