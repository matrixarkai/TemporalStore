...
- The full 1,986-question run is supported by the converter, but the current Rust harness is still
  too slow for quick local iteration because retrieval is repeated per question. The next target is
  a dedicated LOCOMO benchmark binary that ingests each conversation once, streams all questions,
