...
def bounded_baseline_section():
    ...
    specs=[
        ...
        ('LoCoMo OpenViking', base/'openviking_locomo_oss_report.json'),
        ('LongMemEval_s MatrixArk', base/'matrixark_longmemeval_s_oss_report.json'),
        ('LongMemEval_s OpenViking', base/'openviking_longmemeval_s_oss_report.json'),
    ]
    body=['<section class="section"><h2>Bounded Apples-To-Apples Baseline</h2><div class="card"><p>This 100x100 bounded baseline used the same OSS reader/budget contract for MatrixArk and OpenViking/VikingMem-style retrieval. It is not the full run, but it keeps the live checkpoint anchored to prior baseline evidence.</p></div><table><tr><th>Run</th><th>Cases</th><th>Retrieval Hit</th><th>Reader Hit</th><th>Token Reduction</th><th>Retrieval p95</th><th>Reader p95</th></tr>']
