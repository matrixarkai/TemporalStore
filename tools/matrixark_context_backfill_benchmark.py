...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--batch-sizes", default='', help="optional comma-separated batch-size sweep; overrides single --batch-size for benchmark execution")
    parser.add_argument("--payload-bytes", type=int, default=128)
    parser.add_argument("--incremental-records", type=int, default=1000)
    parser.add_argument("--repeat", type=int, default=1, help="number of samples to run for each selected raw backend")
    parser.add_argument("--raw-backends", choices=["both", "temporalstore", "matrixkv"], default=('both'))
    parser.add_argument("--min-full-shadow-qps", type=float, default=0.0)
    parser.add_argument("--min-incremental-repair-qps", type=float, default=0.0)
    parser.add_argument("--min-partial-repair-qps", type=float, default=0.0)
    parser.add_argument("--min-backend-qps-ratio", type=float, default=0.0, help="optional balance floor: slowest selected backend QPS divided by fastest selected backend QPS, 0 disables")
    parser.add_argument("--max-full-shadow-p95-ms", type=float, default=0.0, help="optional p95 latency ceiling for full shadow backfill, 0 disables")
    parser.add_argument("--max-incremental-shadow-p95-ms", type=float, default=0.0, help="optional p95 latency ceiling for incremental shadow build, 0 disables")
    parser.add_argument("--max-incremental-repair-p95-ms", type=float, default=0.0, help="optional p95 latency ceiling for incremental active repair, 0 disables")
    parser.add_argument("--max-partial-shadow-p95-ms", type=float, default=0.0, help="optional p95 latency ceiling for bounded partial shadow backfill, 0 disables")
    parser.add_argument("--max-partial-repair-p95-ms", type=float, default=0.0, help="optional p95 latency ceiling for bounded partial active repair, 0 disables")
    parser.add_argument("--gate-aggregation", choices=["sample", "min", "avg"], default=('min'), help="how performance gates aggregate repeated samples: min is the conservative default, avg smooths noisy local runs, sample checks every sample")
    parser.add_argument("--baseline-json", default='', help="optional prior benchmark JSON used for regression gating")
    parser.add_argument("--min-baseline-qps-ratio", type=float, default=0.0, help="fail when current QPS for a matching backend/batch/phase is below this fraction of baseline, 0 disables")
    parser.add_argument("--max-baseline-latency-ratio", type=float, default=0.0, help="fail when current elapsed latency for a matching backend/batch/phase exceeds this multiple of baseline, 0 disables")
