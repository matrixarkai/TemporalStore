...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--records", type=int, default=int(os.environ.get("MATRIXARK_BACKFILL_BENCH_RECORDS", "").strip() or "10000"))
    parser.add_argument("--batch-size", type=int, default=int(os.environ.get("MATRIXARK_BACKFILL_BENCH_BATCH_SIZE", "").strip() or "1024"))
    ...
    parser.add_argument("--payload-bytes", type=int, default=int(os.environ.get("MATRIXARK_BACKFILL_BENCH_PAYLOAD_BYTES", "").strip() or "128"))
    parser.add_argument("--incremental-records", type=int, default=int(os.environ.get("MATRIXARK_BACKFILL_BENCH_INCREMENTAL_RECORDS", "").strip() or "1000"))
    parser.add_argument("--repeat", type=int, default=int(os.environ.get("MATRIXARK_BACKFILL_BENCH_REPEAT", "").strip() or "1"), help="number of samples to run for each selected raw backend")
    parser.add_argument("--raw-backends", choices=["both", "temporalstore", "matrixkv"], default=(os.environ.get("MATRIXARK_BACKFILL_BENCH_RAW_BACKENDS", "").strip() or "both"))
    parser.add_argument("--min-full-shadow-qps", type=float, default=float(os.environ.get("MATRIXARK_BACKFILL_BENCH_MIN_FULL_SHADOW_QPS", "").strip() or "0"))
    parser.add_argument("--min-incremental-repair-qps", type=float, default=float(os.environ.get("MATRIXARK_BACKFILL_BENCH_MIN_INCREMENTAL_REPAIR_QPS", "").strip() or "0"))
    parser.add_argument("--min-partial-repair-qps", type=float, default=float(os.environ.get("MATRIXARK_BACKFILL_BENCH_MIN_PARTIAL_REPAIR_QPS", "").strip() or "0"))
    parser.add_argument("--min-backend-qps-ratio", type=float, default=float(os.environ.get("MATRIXARK_BACKFILL_BENCH_MIN_BACKEND_QPS_RATIO", "").strip() or "0"), help="optional balance floor: slowest selected backend QPS divided by fastest selected backend QPS, 0 disables")
    parser.add_argument("--max-full-shadow-p95-ms", type=float, default=float(os.environ.get("MATRIXARK_BACKFILL_BENCH_MAX_FULL_SHADOW_P95_MS", "").strip() or "0"), help="optional p95 latency ceiling for full shadow backfill, 0 disables")
    parser.add_argument("--max-incremental-shadow-p95-ms", type=float, default=float(os.environ.get("MATRIXARK_BACKFILL_BENCH_MAX_INCREMENTAL_SHADOW_P95_MS", "").strip() or "0"), help="optional p95 latency ceiling for incremental shadow build, 0 disables")
    parser.add_argument("--max-incremental-repair-p95-ms", type=float, default=float(os.environ.get("MATRIXARK_BACKFILL_BENCH_MAX_INCREMENTAL_REPAIR_P95_MS", "").strip() or "0"), help="optional p95 latency ceiling for incremental active repair, 0 disables")
    parser.add_argument("--max-partial-shadow-p95-ms", type=float, default=float(os.environ.get("MATRIXARK_BACKFILL_BENCH_MAX_PARTIAL_SHADOW_P95_MS", "").strip() or "0"), help="optional p95 latency ceiling for bounded partial shadow backfill, 0 disables")
    parser.add_argument("--max-partial-repair-p95-ms", type=float, default=float(os.environ.get("MATRIXARK_BACKFILL_BENCH_MAX_PARTIAL_REPAIR_P95_MS", "").strip() or "0"), help="optional p95 latency ceiling for bounded partial active repair, 0 disables")
    parser.add_argument("--gate-aggregation", choices=["sample", "min", "avg"], default=(os.environ.get("MATRIXARK_BACKFILL_BENCH_GATE_AGGREGATION", "").strip() or "min"), help="how performance gates aggregate repeated samples: min is the conservative default, avg smooths noisy local runs, sample checks every sample")
    ...
    parser.add_argument("--min-baseline-qps-ratio", type=float, default=float(os.environ.get("MATRIXARK_BACKFILL_BENCH_MIN_BASELINE_QPS_RATIO", "").strip() or "0"), help="fail when current QPS for a matching backend/batch/phase is below this fraction of baseline, 0 disables")
    parser.add_argument("--max-baseline-latency-ratio", type=float, default=float(os.environ.get("MATRIXARK_BACKFILL_BENCH_MAX_BASELINE_LATENCY_RATIO", "").strip() or "0"), help="fail when current elapsed latency for a matching backend/batch/phase exceeds this multiple of baseline, 0 disables")
