...
CORPUS: tuple[MemoryRecord, ...] = (
    MemoryRecord("msg:windows-docker:001", "user:deeproute", "windows-docker-install", "short_term_event", "Windows install should not require WSL runtime. Ship a Docker image and PowerShell installer for Rust TemporalStore, Codex hook setup, and local ports.", ("windows_no_wsl_runtime", "docker_image", "powershell_installer", "codex_hook_setup")),
    MemoryRecord("sum:windows-docker:l1", "user:deeproute", "windows-docker-install", "long_term_summary", "Windows deployment policy: use Docker Desktop with a prepared TemporalStore image; document hook scripts and dependencies; avoid asking users to build inside WSL.", ("windows_no_wsl_runtime", "docker_desktop", "prepared_image", "hook_scripts")),
    MemoryRecord("msg:linux-macos:001", "user:deeproute", "linux-macos-install", "short_term_event", "Linux builds should be recommended on Ubuntu 22.04 and Ubuntu 26.04. Add scripts for Linux and macOS deploy, following the OpenViking-style OSS model setup.", ("ubuntu_2204", "ubuntu_2604", "linux_script", "macos_script", "openviking_style_models")),
    MemoryRecord("ent:oss-model-setup", "user:deeproute", "global", "long_term_entity", "OSS model setup uses tools/install_context_oss_models.sh for Ollama, Qwen, vLLM when available, Transformers, sentence-transformers, and local embedding fallback.", ("install_context_oss_models", "ollama", "qwen", "vllm", "transformers", "embedding_fallback")),
    MemoryRecord("msg:opensource-surface:001", "user:deeproute", "open-source-surface", "short_term_event", "First open-source TemporalStore surface should keep Context, Feature, FeatureAggregate, and Control State capabilities only; remove audit, replay, debug-only models, and broad Redis APIs.", ("context_capability", "feature_capability", "feature_aggregate", "control_state", "no_audit_replay_debug", "minimal_redis")),
    MemoryRecord("ent:control-state", "user:deeproute", "global", "long_term_entity", "Control State stores fast-changing serving signals: counters, caps, quotas, pacing, eligibility, suppression, and risk-control state.", ("control_state", "counters", "caps", "quotas", "pacing", "eligibility", "suppression", "risk_control")),
    MemoryRecord("ent:feature-aggregate", "user:deeproute", "global", "long_term_entity", "FeatureAggregate lives inside Feature. Feature stores time-keyed observations; FeatureAggregate computes serving-time aggregates over those observations, with immature high-cardinality sketches gated.", ("feature_aggregate", "inside_feature", "time_keyed_observations", "serving_time_aggregates", "gate_high_cardinality_sketches")),
    MemoryRecord("msg:codex-hook:001", "user:deeproute", "codex-hook-realtime", "short_term_event", "Codex hooks must fire from real UserPromptSubmit events across all old and new conversations. Do not use watcher/log fallback as the serving source.", ("user_prompt_submit", "all_conversations", "no_watcher_serving_source", "realtime_hook")),
    MemoryRecord("sum:codex-hook:l1", "user:deeproute", "codex-hook-realtime", "long_term_summary", "Hook invariant: global hook registration should ingest real user prompts to both Rust and C++ TemporalStore, preserving session/thread identity and enabling query by all sessions.", ("global_hook_registration", "rust_and_cpp", "preserve_session_identity", "query_all_sessions")),
    MemoryRecord("msg:perf-parity:001", "user:deeproute", "performance-parity", "short_term_event", "For C++ vs Rust performance claims, do not use retired matrixark_record_log bridge artifacts. Use direct SDK or mature proxy paths and require selected refs to be non-empty before latency claims.", ("no_matrixark_record_log", "direct_sdk_or_proxy", "selected_refs_non_empty", "fair_perf_claims")),
)
...
def retrieve_context(case: QueryCase, limit: int = 4) -> list[MemoryRecord]:
    ...
    for rec in CORPUS:
        ...
        if rec.scope == "user:deeproute" and score > 0:
