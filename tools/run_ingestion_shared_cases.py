...
native execution remains optional through ``TS_NATIVE_INGESTION_NATIVE_CMD``;
...
def main() -> int:
    ...
    parser.add_argument(
        "--native-native",
        action="store_true",
        help="run TS_NATIVE_INGESTION_NATIVE_CMD for every ingestion case",
    )
    parser.add_argument(
        "--require-native-native",
        action="store_true",
        help="fail unless TS_NATIVE_INGESTION_NATIVE_CMD is configured",
    )
    ...
    native_template = os.environ.get("TS_NATIVE_INGESTION_NATIVE_CMD")
    if args.require_native and not native_template:
        raise SystemExit("TS_NATIVE_INGESTION_NATIVE_CMD is required for native ingestion execution")
    if args.native_native:
        if not native_template:
            raise SystemExit("set TS_NATIVE_INGESTION_NATIVE_CMD to run ingestion cases")
        cwd = native_repo or ROOT
        for case, location, _ in case_commands:
            print(f"== native ingestion case: {location} ==")
