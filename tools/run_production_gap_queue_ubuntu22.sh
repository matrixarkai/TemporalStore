...
register_gap 09 "Cache/storage fallback metrics" quick partial "run_prometheus_local_ubuntu22.sh" "${prometheus_command}" "Page-store read path now emits blockcache get, blockcache hit, and persistent read counters; local Prometheus validation asserts the persistent-read sample is exported."
