...
rebalance_command='env BUILD_TYPE="${BUILD_TYPE}" RESULT_DIR="${RESULT_DIR}/rebalance" BASE_MS_PORT="${BASE_MS_PORT}" BASE_SERVER_PORT="${BASE_SERVER_PORT}" OPS=240 THREADS=2 RUN_DATA_RAFT_REBALANCE=1 RUN_SHARED_STORE_REBALANCE=1 bash tools/run_rebalance_local_ubuntu22.sh'
...
data_primary_kill_scale_command='env BUILD_TYPE="${BUILD_TYPE}" RESULT_DIR="${RESULT_DIR}/data_primary_kill_scale" OPS=240 THREADS=2 SCALE_UP_BACKGROUND_OPS=480 SCALE_UP_BACKGROUND_THREADS=2 KILL_PRIMARY_DURING_SCALE_UP=1 bash tools/run_data_raft_scale_up_down_ubuntu22.sh'
...
register_gap 02 "Data primary kill during scale up/down" pr covered "run_data_raft_scale_up_down_ubuntu22.sh" "${data_primary_kill_scale_command}" "Interleaved gate runs client traffic, adds a third data node, kills the current data primary during scale-up convergence, validates promoted-primary serving, records bounded transient client/proxy errors, and accepts a stale scale-down UpdateTable response only after topology has already converged to a healthy two-voter serving set. Refreshed release binary validation passed locally."
...
register_gap 05 "Dedicated rebalance harness" pr covered "tools/run_rebalance_local_ubuntu22.sh" "${rebalance_command}" "Dedicated harness runs both data-raft add/remove redistribution and shared-store three-node scale profiles, records before/after topology evidence, and emits Prometheus textfile metrics for harness and per-case pass/fail state."
...
register_gap 10 "5-node data raft local scale" nightly covered "run_data_raft_5node_scale_ubuntu22.sh" "${data_raft_5node_command}" "Five data-node raft cluster gate starts 5 servers with 3 replicas, validates write/read plus replication smoke, checks replica distribution, and emits Prometheus scale metrics."
...
register_gap 12 "Snapshot restore under write pressure" pr covered "run_data_raft_snapshot_restore_ubuntu22.sh" "${raft_pr_command}" "Data snapshot restore gate now runs concurrent write pressure during snapshot trigger, validates the pressure writes, restarts a follower from existing raft/storage directories, and verifies post-restore serving."
register_gap 13 "Metaserver snapshot restore gate" pr covered "run_metaserver_raft_snapshot_restore_ubuntu22.sh" "${metaserver_snapshot_command}" "Dedicated local gate triggers metaserver raft snapshot, restarts all metaserver nodes from existing work dirs, verifies membership and metadata survive, writes again after restore, and emits Prometheus restore evidence."
...
register_gap 16 "Process restart with stale local data gate" pr covered "tools/run_stale_local_data_restart_gate_ubuntu22.sh" "${stale_restart_command}" "Dedicated gate exercises data-node restart from existing raft/storage directories after writes, verifies persisted applied-index/WAL evidence, emits Prometheus metrics, and checks stale/corrupt applied-index guard coverage."
...
register_gap 20 "Build/test CI with dependency cache" quick covered "tools/run_dependency_cache_gate_ubuntu22.sh" "${dependency_cache_command}" "Dedicated CI gate validates Ubuntu 22 build tools, local byte/rustraft dependency-cache symlinks, build-script cache knobs, cached release-binary evidence, optional reusable system/static libraries, and Prometheus textfile metrics."
