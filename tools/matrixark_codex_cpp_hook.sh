...
if [[ "$MATRIXARK_HOOK_AUTOSTART_CPP" == "1" && "$MATRIXARK_MCP_BACKEND" == "temporalstore-direct" ]]; then
  ...
  if ! timeout 1 bash -c "</dev/tcp/$host/$port" >/dev/null 2>&1; then
    if ! BUILD_TYPE="${BUILD_TYPE:-Release}" DEPLOY_DIR="$MATRIXARK_CPP_DEPLOY_DIR" PERSIST_DEPLOY_DIR=1 METASERVER_EXTRA_FLAGS="$MATRIXARK_CPP_METASERVER_EXTRA_FLAGS" SERVER_EXTRA_FLAGS="$MATRIXARK_CPP_SERVER_EXTRA_FLAGS" timeout 30 bash "$ROOT/tools/deploy_local_ubuntu22.sh" start >/dev/null 2>&1; then
      BUILD_TYPE="${BUILD_TYPE:-Release}" DEPLOY_DIR="$MATRIXARK_CPP_DEPLOY_DIR" PERSIST_DEPLOY_DIR=0 METASERVER_EXTRA_FLAGS="$MATRIXARK_CPP_METASERVER_EXTRA_FLAGS" SERVER_EXTRA_FLAGS="$MATRIXARK_CPP_SERVER_EXTRA_FLAGS" timeout 30 bash "$ROOT/tools/deploy_local_ubuntu22.sh" start >/dev/null 2>&1 || true
    fi
  fi
  _matrixark_apply_local_cpu_bounds
fi
