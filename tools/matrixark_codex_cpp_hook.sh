...
export MATRIXARK_CODEX_CPP_USER_PROMPTS_ONLY="${MATRIXARK_CODEX_CPP_USER_PROMPTS_ONLY:-0}"
export MATRIXARK_HOOK_COMPACT_HOT_PREFIX_ONLY="${MATRIXARK_HOOK_COMPACT_HOT_PREFIX_ONLY:-1}"
...
_matrixark_hook_event="${1:-UserPromptSubmit}"
if [[ "$_matrixark_hook_event" == "--event" ]]; then
  _matrixark_hook_event="${2:-UserPromptSubmit}"
fi
if [[ "$MATRIXARK_CODEX_CPP_USER_PROMPTS_ONLY" == "1" && "$_matrixark_hook_event" != "UserPromptSubmit" ]]; then
  exit 0
fi
