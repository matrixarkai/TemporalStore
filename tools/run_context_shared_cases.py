...
be supplied later through ``TS_NATIVE_CONTEXT_NATIVE_CMD`` while this runner keeps
...
def main() -> int:
    ...
    parser.add_argument("--native-native", action="store_true")
    parser.add_argument("--require-native-native", action="store_true")
    ...
    native_template = os.environ.get("TS_NATIVE_CONTEXT_NATIVE_CMD")
    if args.require_native and not native_template:
        raise SystemExit("TS_NATIVE_CONTEXT_NATIVE_CMD is required for native context execution")
    if args.native_native:
        if not native_template:
            raise SystemExit("set TS_NATIVE_CONTEXT_NATIVE_CMD to run context cases")
        cwd = native_repo or ROOT
        for case, location, _ in case_commands:
            print(f"== native context case: {location} ==")
