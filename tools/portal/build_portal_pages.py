...
# The shared block is delimited so `inject()` can REPLACE it on the two hand-maintained pages
# rather than only adding it once. Without the markers the refresh path had no way to find what it
# had written the first time, so those pages kept whatever this constant said years of edits ago.
NAV_CSS_START = "  /* shared portal css: generated by build_portal_pages.py */"
NAV_CSS_END = "  /* end shared portal css */"
...
# extraction carries that block back out again -- and the generated pages, which append NAV_CSS
# themselves, end up with two copies. Worse, the copy they get is the one written on the PREVIOUS
# run, so any edit to NAV_CSS took two builds to settle and the committed pages were a build behind.
# Stripped here, at the only place the two can be told apart.
if NAV_CSS_START in BASE_CSS and NAV_CSS_END in BASE_CSS:
    _start = BASE_CSS.index(NAV_CSS_START)
    _end = BASE_CSS.index(NAV_CSS_END, _start) + len(NAV_CSS_END)
    BASE_CSS = BASE_CSS[:_start].rstrip("\n") + BASE_CSS[_end:]

# What the first injection wrote before there were markers, ending at the last rule it contained.
# Recognised so it can be replaced instead of left underneath the new block.
NAV_CSS_LEGACY_END = ("  .portalnav a[aria-current=page]"
...
EXTRA_CSS = """
  ...
  /* A table whose row headers cannot wrap is as wide as its widest header, and without this the
     page body carries that width: at 375px this page's document scrolled to 579px. The table
     scrolls inside its own panel instead. */
  .tablewrap{overflow-x:auto;max-width:100%}
  /* The table must be allowed to EXCEED the wrapper, or the wrapper has nothing to scroll and the
     table squeezes instead. `.inv` is width:100%, which a browser treats as preferred and exceeds
     only as far as minimum content widths force -- and `.inv th` is nowrap while `.inv td` breaks
     anywhere, so the settled shape is "row header in full, value in a sliver". Measured at 271px
     before this rule: a 199px header, a 7px value cell, and one row 865px tall. */
  .tablewrap .inv{min-width:28rem}
...
(function () {
  ...
  function renderBudgets(budgets) {
    ...
    /* The agent-hook row is one number for two implementations. The shared pipeline resolves it
       the way this panel does; the self-contained offline engine reads the same variable at its
       own site and falls back to a different number. Which one an agent gets turns on whether a
       proxy binary exists where the hook runs, which this deployment cannot see from here.

       Said only when the variable is unset, because that is the case where they certainly differ.
       With it set, both backends read it and ordinary values agree. */
    if (budgets.hook_budget_is_set === false) {
      note += '<p class="hint"><strong>The agent-hook budget is not set, so the two hook '
            + "backends fall back to different numbers.</strong> This row is what the shared "
            + "pipeline resolves; the self-contained offline engine falls back to its own. Set "
            + '<span class="mono">MATRIXARK_HOOK_MAX_CONTEXT_TOKENS</span> to a plain whole '
            + "number and both read it.</p>";
    }
    ...
    $("monitoring").innerHTML = "<table><thead><tr><th>Download</th><th>Covers</th>" +
      "<th>Scraped from</th></tr></thead><tbody>" + assets.map(function (a) {
        ...
        return "<tr><td><button class='link' type='button' data-asset='" + esc(a.asset) +
          ...
          "<td>" + esc(a.covers) + "</td>" +
          /* `scraped_from` is for an asset that belongs to no single target: rules on
             `up` apply to every job at once, and giving them a target of their own
             would put a job that scrapes nothing into the config copied out below. */
          "<td>" + esc(a.scraped_from || target.label || a.scrape) + "<div class='hint mono' style='margin:2px 0 0'>" +
...
%(nav)s
  ...
  <section>
    ...
    <p class="hint" style="margin-top:0">How a candidate is scored. This is the switch that decides
      what "close to the question" means on this deployment, and it is on by default. Read by
      running the same resolution a retrieve runs, so it is what applies rather than what is
      offered.</p>
  ...
  <section>
    ...
    <p class="hint" style="margin-top:0">The three caps that bound a retrieve, and which of them was
      doing the cutting. Raising the others changed nothing in that measurement. Each shows the
      value in force and the build default it fell back to, so an override is visible as one.</p>
...
(function () {
  ...
  /* These two panels used to ask the settings registry which of six named knobs it offered, and
     render those. That answers "is this configurable here", which is not the question either
     heading asks -- and the two answers were never the same, the registry's ref cap disagreeing
     with the served one by up to 156x. When the registry stopped offering the six, both panels
     said "This build offers none of them." about a profile that was deciding every result and
     caps that were cutting every retrieve.

     /v1/admin/retrieval runs the resolution the serving path runs. A value it could not read
     comes back as known:false, and is shown as unread rather than as a default. */
  /* Which level supplied a value, in the words both endpoints use. The caps come from
     /v1/admin/retrieval and the return-all knobs from /v1/admin/policy, and both answer with the
     same three: a per-user override, a tenant override, the environment variable, or nothing set
     at all -- which gets no badge, because "the build decided" is the quiet case and a badge on
     every row is a badge nobody reads.

     The neutral bordered badge, not the amber "restart" one: nothing here needs a restart, both
     levels resolve per call, and an amber badge on a healthy deployment teaches a reader to stop
     seeing amber. */
  var LEVEL = {
    user: "set for this user",
    tenant: "set by tenant policy",
    env: "set by the variable"
  };

  function levelBadge(source) {
    return LEVEL[source]
      ? '<span class="badge override">' + esc(LEVEL[source]) + "</span>" : "";
  }

  function unreadable(data, what) {
  ...
  /* How many workers this deployment was started with, off the same scrape.

     Null when the series is absent. Today that renders the same as a count of 1, because the
     sentence below only quantifies above one -- so this is not a distinction the panel currently
     makes, and a test cannot see it. It is null anyway for the next reader: a fabricated 1 would
     be wrong the moment somebody divides by it. */
  function workerCount(text) {
    var m = /^matrixark_gateway_workers\s+([0-9]+)/m.exec(String(text || ""));
  ...
  function renderAnswering(text) {
    ...
    /* The caveat, quantified where the scrape allows it. Not multiplied out: four workers do not
       answer alike, and scaling one worker's counts by the worker count would invent a
       deployment-wide total from one sample. */
    var workers = workerCount(text);
    var scope = workers && workers > 1
      ? "This worker only, since it started \u2014 one of " + workers + "."
      : "This worker only, since it started.";
    var rows = [
    ...
    return head + "<div class='tablewrap'><table class='inv'><thead><tr><th>Outcome</th>"
      ...
      + '<p class="hint">' + esc(scope) + " Two workers no more add up to the deployment's "
      + "answers than their resident sets do, so no total is offered here that would be wrong."
      + "</p>";
  }

  function renderProfile(data) {
    var said = unreadable(data, "This deployment could not be asked how it scores a candidate.");
    if (said) { return said; }
    var p = (data && data.profile) || {};
    var dense = !!p.embedding_first;
    /* The sentence first, then the numbers. Somebody opening this page wants to know which of two
       things the deployment does; the weights are the evidence, not the answer. */
    var summary = dense
      ? "A candidate's score is its vector similarity alone."
      : "A candidate's score blends vector similarity with a lexical match.";
    var rows = [
      ["Scoring", summary],
      /* Two decimals in both states. Printed raw, the dense case is "1" and "0" while the
         blended one is "0.72" and "0.28", so one switch formats its two positions differently
         and the dense case reads as a flag rather than as the weights it is. */
      ["Vector weight", Number(p.dense_weight).toFixed(2)],
      ["Lexical weight", Number(p.lexical_weight).toFixed(2)],
      ["Variable", String(p.env || "")],
      ["At the build default", p.embedding_first === p.default_is_on ? "yes" : "no — this was set"],
      ["Scan carries only what it reads", p.scan_projection ? "yes" : "no"]
    ].map(function (pair) {
      return "<tr><th>" + esc(pair[0]) + "</th><td class='mono'>" + esc(pair[1]) + "</td></tr>";
    }).join("");
    return "<div class='tablewrap'><table class='inv'><tbody>" + rows
      + "</tbody></table></div>";
  }
  ...
  function renderCaps(data) {
    ...
    if (!caps.length) { return '<div class="empty">This build declares no retrieval caps.</div>'; }
    var rows = caps.map(function (c) {
      /* An applied value differing from the build default means somebody set it -- which the old
         registry view could not show at all, because it only ever had the declared number. */
      /* WHICH level set it, not merely that something did. A tenant override beats the
         environment variable, so naming the variable beside a value the variable did not supply
         sends an operator to change something that will not take effect.

         The neutral badge, not the amber "restart" one: nothing here needs a restart -- both
         levels resolve per call -- and an amber badge on a healthy deployment is a small
         permanent falsehood that teaches the reader to stop seeing amber. */
      var set = levelBadge(c.source);
      var flag = c.cuts
        ? '<span class="badge essential">this is the one that cut it</span>' : "";
      return "<tr><th>" + esc(c.name) + set + flag + "</th><td class='mono'>" + esc(c.value)
        + "</td><td class='mono'>" + esc(c.build_default) + "</td><td class='mono'>"
        + esc(c.env || "") + "</td></tr>"
        /* Said on the row it applies to. A tenant override makes the variable in the last
           column inert, and a reader who does not know that reads the column as the way to
           change the number. */
        + (c.source === "tenant"
           ? "<tr><td colspan='4' class='hint'>A tenant override supplies this, so "
             + esc(c.env) + " is not consulted. Change it under Retrieval settings on the Setup "
             + "page, or clear the override for the variable to apply again.</td></tr>"
           : "")
        + (c.help ? "<tr><td colspan='4' class='hint'>" + esc(c.help) + "</td></tr>" : "");
    }).join("");
    return "<div class='tablewrap'><table class='inv'><thead><tr><th>Cap</th>"
      + "<th>In force</th><th>Build default</th><th>Variable</th></tr></thead><tbody>"
  ...
  function renderVectors(store) {
    ...
    var table = rows
      ? "<div class='tablewrap'><table class='inv'><thead><tr><th>Written by</th>"
        + "<th>Width</th><th>Vectors</th></tr></thead><tbody>" + rows + "</tbody></table></div>"
  ...
  function renderPolicy(knobs) {
    ...
    var rows = names.map(function (name) {
      ...
      /* The endpoint has always sent `source` for every knob and this panel dropped it, so a row
         read as "this is the value" without saying who set it -- the same thing the cap panel was
         doing, except here the answer was already on the wire. */
      return "<tr><th>" + esc(name) + inert + levelBadge(k.source) + "</th><td class='mono'>"
        + esc(k.value == null ? k["default"] : k.value) + "</td><td class='hint'>"
        + esc(String(k.description || "").split("\n")[0]) + "</td></tr>";
    }).join("");
    return rows
      ? "<div class='tablewrap'><table class='inv'><thead><tr><th>Knob</th><th>Value</th>"
  ...
  function load() {
    ...
    /* One call for both panels, and a small one. This used to pull the whole settings registry
       to render six rows out of it. */
    fetch("/v1/admin/retrieval", { headers: auth() })
      .then(function (r) { return r.ok ? r.json() : Promise.reject(r.status); })
      .then(function (d) {
        conn("live", "connected");
...
def _with_nav_css(text):
    """Put the current shared CSS in, replacing whatever is there.

    Three cases, in the order they are met: a page already carrying the delimited block (replace
    between the markers), a page carrying the undelimited block the first injection wrote (replace
    from its comment through the last rule it had), and a page with neither (insert before the end
    of its first stylesheet).
    """
    block = NAV_CSS_START + NAV_CSS.rstrip("\n") + "\n" + NAV_CSS_END + "\n"
    if NAV_CSS_START in text:
        start = text.index(NAV_CSS_START)
        end = text.index(NAV_CSS_END, start) + len(NAV_CSS_END) + 1
        return text[:start] + block + text[end:]
    legacy = "  /* shared portal nav (same markup on every page"
    if legacy in text and NAV_CSS_LEGACY_END in text:
        start = text.index(legacy)
        end = text.index(NAV_CSS_LEGACY_END, start) + len(NAV_CSS_LEGACY_END) + 1
        return text[:start] + block + text[end:]
