...
def collect_task_evidence(workspace: Path) -> dict[str, Any]:
    task_root = workspace / (os.environ.get("EXTERNAL_BASELINE_WORKSPACE_SUBDIR", "").strip() or "workspace") / "default" / "_system" / "tasks"
    user_root = workspace / (os.environ.get("EXTERNAL_BASELINE_WORKSPACE_SUBDIR", "").strip() or "workspace") / "default" / "user"
