...
if which clang-format >&/dev/null; then
    CLANG_FORMAT_BINARY="clang-format"
elif which clang-format-4.0 >&/dev/null; then
    CLANG_FORMAT_BINARY="clang-format-4.0"
elif which clang-format-6.0 >&/dev/null; then
    CLANG_FORMAT_BINARY="clang-format-6.0"
else
...
do
    echo $file
    $CLANG_FORMAT_BINARY -style=$FORMAT $file > $TMPFILE
