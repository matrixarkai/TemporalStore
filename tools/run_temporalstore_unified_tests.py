...
def validate_matrixraft_report_contract(path: Path, case: dict, step: dict) -> None:
    if "matrixraft" not in case["name"].lower() and "rustraft" not in case["name"].lower():
        return
    location = f"{path}: case {case['name']} step {step['name']}"
    command = step["command"]
    contract = command.get("report_contract")
    if not isinstance(contract, dict):
        raise SystemExit(f"{location}: MatrixRaft shared case must declare report_contract")
