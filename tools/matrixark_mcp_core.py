...
# The retrieval budget constants above are imported, not redefined: matrixark_mcp_runtime_config is
# the single source of truth. They used to be declared in BOTH modules, and the 500k-window retune
# updated only runtime_config -- so whichever module a call site happened to import from decided the
# budget (top-k 8 vs 24, selected refs 64 vs 1000, min score 0.20 vs 0.05). Same env var names, so a
# deployment override still wins.
