...
def profile_entity_type_for_memory_text(text: str) -> str:
    ...
    if re.search(r"\b(?:feature parity|feature[- ]focused|features? only|features? referring to|functionality|functionalities|functionality only|algorithms?|implementation focus|no testing|no tests?|skip tests?|without tests?|no monitoring|no debugging|no debug|no evidence|no evident|no eviden[ct]e|feature work only|code changes only|openviking|vikingmem|mem0|long[- ]term memory|session memory|profile memory|cross[- ]session memory|threshold|idle batch|batch extraction)\b", lower):
...
def deterministic_secondary_index_filter_groups(query: str, question_type: str) -> list[set[str]]:
    ...
    if re.search(r"\b(workspace|repo|repository|branch|remote|github|origin/main|main branch|ubuntu|wsl|linux|windows folder|worktree|build|deploy|deployment|rustraft|temporalstore|matrixark)\b", lower):
        ...
        add_group(context_index_name("profile_memory_kind", "durable_profile"))
        add_group(context_index_name("memory_scope", "user_profile"))
    if re.search(r"\b(openviking|vikingmem|mem0|feature parity|feature[- ]focused|features? only|features? referring to|functionality|functionalities|functionality only|algorithms?|implementation focus|no testing|no tests?|skip tests?|without tests?|no monitoring|no debugging|no debug|no evidence|no evident|no eviden[ct]e|feature work only|code changes only|session memory|profile memory|cross[- ]session memory|threshold|idle batch|batch extraction)\b", lower):
