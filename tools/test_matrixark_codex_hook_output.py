...
class MatrixArkCodexHookOutputTest(unittest.TestCase):
    ...
    def test_fast_async_hook_ingest_writes_raw_and_serving_scope(self) -> None:
        ...
        self.assertTrue(all(record["source_memory_scopes"] == ["session"] for record in dirty_records))
        self.assertTrue(all(record["source_session_continuities"] == ["same_session"] for record in dirty_records))
