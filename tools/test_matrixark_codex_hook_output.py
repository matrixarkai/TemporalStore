...
class MatrixArkCodexHookOutputTest(unittest.TestCase):
    ...
    def test_fast_async_hook_ingest_writes_raw_and_serving_scope(self) -> None:
        ...
        self.assertEqual(["user"], embedding_records[0]["source_roles"])
        self.assertEqual({"user": 1}, embedding_records[0]["source_role_counts"])
        self.assertEqual(["before_llm"], embedding_records[0]["source_hook_types"])
        self.assertEqual({"before_llm": 1}, embedding_records[0]["source_hook_type_counts"])
        self.assertEqual(["UserPromptSubmit"], embedding_records[0]["source_codex_events"])
        self.assertEqual({"UserPromptSubmit": 1}, embedding_records[0]["source_codex_event_counts"])
        self.assertEqual(["session", "user_profile"], embedding_records[0]["source_memory_scopes"])
        self.assertEqual(["same_session", "cross_session"], embedding_records[0]["source_session_continuities"])
        self.assertEqual(["pending_async"], embedding_records[0]["source_extraction_phases"])
        self.assertEqual([serving["event_id_hash"]], embedding_records[0]["source_event_ids"])
