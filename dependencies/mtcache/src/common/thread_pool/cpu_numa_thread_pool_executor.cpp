...
#include "cpu_numa_thread_pool_executor.h"
...
using default_queue =
    folly::UnboundedBlockingQueue<CPUNumaThreadPoolExecutor::CPUTask>;
using default_queue_alloc =
    folly::AlignedSysAllocator<default_queue,
                               folly::FixedAlign<alignof(default_queue)>>;
template <class T>
using BlockingQueue = folly::BlockingQueue<T>;
using ExecutorBlockingGuard = folly::ExecutorBlockingGuard;

constexpr folly::StringPiece executorName = "CPUNumaThreadPoolExecutor";
}  // namespace

const size_t CPUNumaThreadPoolExecutor::kDefaultMaxQueueSize = 1 << 14;

CPUNumaThreadPoolExecutor::CPUNumaThreadPoolExecutor(
    size_t numThreads, bool dynamicNumThreads,
    std::unique_ptr<BlockingQueue<CPUTask>> taskQueue,
    std::shared_ptr<ThreadFactory> threadFactory)
    : NumaThreadPoolExecutor(numThreads, dynamicNumThreads ? 0 : numThreads,
...
CPUNumaThreadPoolExecutor::CPUNumaThreadPoolExecutor(
    ...
    std::unique_ptr<BlockingQueue<CPUTask>> taskQueue,
    std::shared_ptr<ThreadFactory> threadFactory)
    : NumaThreadPoolExecutor(numThreads.first, numThreads.second,
                             std::move(threadFactory)),
      taskQueue_(taskQueue.release()) {
...
CPUNumaThreadPoolExecutor::CPUNumaThreadPoolExecutor(size_t numThreads,
                                                     bool dynamicNumThreads)
    : CPUNumaThreadPoolExecutor(
          numThreads, dynamicNumThreads,
          std::make_shared<NamedThreadFactory>("CPUThreadPool")) {}

CPUNumaThreadPoolExecutor::CPUNumaThreadPoolExecutor(
    size_t numThreads, bool dynamicNumThreads, int8_t numPriorities,
    std::shared_ptr<ThreadFactory> threadFactory)
    : CPUNumaThreadPoolExecutor(
          numThreads, dynamicNumThreads,
          std::make_unique<folly::PriorityUnboundedBlockingQueue<CPUTask>>(
              numPriorities),
          std::move(threadFactory)) {}

CPUNumaThreadPoolExecutor::CPUNumaThreadPoolExecutor(
    size_t numThreads, bool dynamicNumThreads, int8_t numPriorities,
    size_t maxQueueSize, std::shared_ptr<ThreadFactory> threadFactory)
    : CPUNumaThreadPoolExecutor(
          numThreads, dynamicNumThreads,
          std::make_unique<folly::PriorityLifoSemMPMCQueue<CPUTask>>(
...
folly::QueueObserver* FOLLY_NULLABLE
CPUNumaThreadPoolExecutor::getQueueObserver(int8_t pri) {
  ...
  folly::QueueObserver* existingObserver = nullptr;
...
void CPUNumaThreadPoolExecutor::add(Func func) {
...
void CPUNumaThreadPoolExecutor::addWithPriority(Func func, int8_t priority) {
  add(std::move(func), priority, std::chrono::milliseconds(0));
}

void CPUNumaThreadPoolExecutor::add(Func func, int8_t priority,
                                    std::chrono::milliseconds expiration,
                                    Func expireCallback) {
  CHECK(getNumPriorities() > 0);
  CPUTask task(std::move(func), expiration, std::move(expireCallback),
               priority);
...
uint8_t CPUNumaThreadPoolExecutor::getNumPriorities() const {
  return taskQueue_->getNumPriorities();
}

size_t CPUNumaThreadPoolExecutor::getTaskQueueSize() const {
  return taskQueue_->size();
}

BlockingQueue<CPUNumaThreadPoolExecutor::CPUTask>*
CPUNumaThreadPoolExecutor::getTaskQueue() {
  return taskQueue_.get();
}

// threadListLock_ must be writelocked.
bool CPUNumaThreadPoolExecutor::tryDecrToStop() {
...
bool CPUNumaThreadPoolExecutor::taskShouldStop(folly::Optional<CPUTask>& task) {
  if (tryDecrToStop()) {
    return true;
  }
  if (task) {
...
void CPUNumaThreadPoolExecutor::threadRun(ThreadPtr thread) {
...
void CPUNumaThreadPoolExecutor::stopThreads(size_t n) {
  threadsToStop_ += n;
...
size_t CPUNumaThreadPoolExecutor::getPendingTaskCountImpl() const {
  return taskQueue_->size();
}

std::unique_ptr<folly::QueueObserverFactory>
CPUNumaThreadPoolExecutor::createQueueObserverFactory() {
  for (auto& observer : queueObservers_) {
    observer.store(nullptr, std::memory_order_release);
  }
  return folly::QueueObserverFactory::make("cpu." + getName(),
