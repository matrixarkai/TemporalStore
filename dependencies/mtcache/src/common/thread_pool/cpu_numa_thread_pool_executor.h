 ...
 * folly-v2021.03.08.00/folly/executors/CPUThreadPoolExecutor.h. They are mostly
 ...
 */
class CPUNumaThreadPoolExecutor : public NumaThreadPoolExecutor {
 public:
  template <class T>
  ...
  // Construct CPUNumaThreadPoolExecutor with the number of threads and the
  // flag whether to dynamically adjust the active thread number.
  //
  // If `dynamicNumThreads` is true, this executor will destroy the threads
  // when they are idle for at least FLAGS_numa_threadtimeout_ms and
  // re-create the threads when new tasks come. Otherwise the idle threads
  // will not be destroyed.
  // It will save some memory if `dynamicNumThreads` is true when idle threads
  // are destroyed.
  //
  // Be careful to set `dynamicNumThreads` to true if you want to use the
  // feature of `setCpuAffinity`. Because there is a short delay for the CPU
  // affinity to take effect when the threads are added. If `dynamicNumThreads`
  // is true, the re-created threads may not follow the CPU affinity
  // immediately when new tasks come.
  CPUNumaThreadPoolExecutor(size_t numThreads, bool dynamicNumThreads);

  CPUNumaThreadPoolExecutor(size_t numThreads, bool dynamicNumThreads,
                            std::shared_ptr<ThreadFactory> threadFactory);

  CPUNumaThreadPoolExecutor(std::pair<size_t, size_t> numThreads,
                            std::shared_ptr<ThreadFactory> threadFactory);
  ...
  ~CPUNumaThreadPoolExecutor() override;

  void add(Func func) override;
  void add(Func func, std::chrono::milliseconds expiration,
           Func expireCallback = nullptr) override;

  void addWithPriority(Func func, int8_t priority) override;
  void add(Func func, int8_t priority, std::chrono::milliseconds expiration,
  ...
  struct CPUTask : public NumaThreadPoolExecutor::Task {
    // Must be noexcept move constructible so it can be used in MPMCQueue

    explicit CPUTask(Func&& f, std::chrono::milliseconds expiration,
                     Func&& expireCallback, int8_t pri)
 ...
 protected:
  BlockingQueue<CPUTask>* FOLLY_NULLABLE getTaskQueue();
 ...
 private:
  ...
  folly::QueueObserver* FOLLY_NULLABLE getQueueObserver(int8_t pri);
