...
template <
    typename KeyType, typename ValueType, typename HashFn = std::hash<KeyType>,
    typename KeyEqual = std::equal_to<KeyType>,
    typename Allocator = std::allocator<uint8_t>, uint8_t ShardBits = 8,
    template <typename> class Atom = std::atomic, class Mutex = std::mutex,
...
class ConcurrentHashMap {
  using SegmentT =
      detail::ConcurrentHashMapSegment<KeyType, ValueType, ShardBits, HashFn,
                                       KeyEqual, Allocator, Atom, Mutex, Impl>;
  template <typename K, typename T>
  using EnableHeterogeneousFind =
      std::enable_if_t<::folly::detail::EligibleForHeterogeneousFind<
                           KeyType, HashFn, KeyEqual, K>::value,
                       T>;
 ...
 public:
  ...
  ConcurrentHashMap& operator=(ConcurrentHashMap&& o) {
    for (uint64_t i = 0; i < NumShards; i++) {
      ...
      if (seg) {
        ...
        Allocator().deallocate(reinterpret_cast<uint8_t*>(seg),
                               sizeof(SegmentT));
      }
      segments_[i].store(o.segments_[i].load(std::memory_order_relaxed),
                         std::memory_order_relaxed);
  ...
  // TODO(kai) return the segment, then no need to call
  // ensureSegment again in unlock, insert....
  // But has to change the interface of the above functions
  // Disscuss with group later.
  // Attempt to acquire segment lock
  template <typename Key>
  bool map_trylock(Key&& k) {
    auto segment = pickSegment(k);
    return ensureSegment(segment)->map_trylock();
  }

  template <typename Key>
  void map_unlock(Key&& k) {
    auto segment = pickSegment(k);
    ensureSegment(segment)->map_unlock();
  }

  template <typename Key>
  void map_lock(Key&& k) {
    auto segment = pickSegment(k);
    ensureSegment(segment)->map_lock();
  }

  template <typename Key>
  Mutex* get_map_lock(Key&& k) {
    auto segment = pickSegment(k);
    return ensureSegment(segment)->get_map_lock();
  }
  ...
  std::pair<ConstIterator, bool> emplace(Args&&... args) {
    ...
    std::pair<ConstIterator, bool> res(std::piecewise_construct,
                                       std::forward_as_tuple(this, segment),
                                       std::forward_as_tuple(false));
    res.second = ensureSegment(segment)->emplace(res.first.it_,
                                                 node->getItem().first, node);
    if (!res.second) {
      node->~Node();
  ...
  // cache: must hold a lock first
  /*
   * The bool component will return true if an insertion operation is performed,
   * or false if an assignment operation is performed. Note that this behavior
   * is now consistent with that of std::unordered_map::insert_or_assign, but
   * different from upstream folly::concurrenthashmap::insert_or_assign.
   */
  template <typename Key, typename Value>
  ...
  ::folly::Optional<ConstIterator> assign_if_equal(Key&& k,
    ...
    if (!seg) {
      ...
      auto r = seg->assign_if_equal(res.it_, std::forward<Key>(k), expected,
 ...
 private:
  ...
  const ValueType atImpl(const K& k) const {
    auto item = find(k);
    if (item == cend()) {
      ::folly::throw_exception<std::out_of_range>("at(): key not in map");
    }
    return item->second;
  }
  ...
  SegmentT* ensureSegment(uint64_t i) const {
    ...
    if (!seg) {
      ...
      SegmentT* newseg =
          reinterpret_cast<SegmentT*>(Allocator().allocate(sizeof(SegmentT)));
      newseg = new (newseg)
      ...
      if (!segments_[i].compare_exchange_strong(seg, newseg)) {
        ...
        Allocator().deallocate(reinterpret_cast<uint8_t*>(newseg),
                               sizeof(SegmentT));
      } else {
        seg = newseg;
      }
  ...
  ::folly::hazptr_obj_cohort<Atom>* cohort() const noexcept {
    return cohort_.load(std::memory_order_acquire);
  }

  ::folly::hazptr_obj_cohort<Atom>* ensureCohort() const {
    auto b = cohort();
    if (!b) {
      auto storage =
          Allocator().allocate(sizeof(::folly::hazptr_obj_cohort<Atom>));
      auto newcohort = new (storage)::folly::hazptr_obj_cohort<Atom>();
      if (cohort_.compare_exchange_strong(b, newcohort)) {
        b = newcohort;
      } else {
        newcohort->~hazptr_obj_cohort();
        Allocator().deallocate(storage,
                               sizeof(::folly::hazptr_obj_cohort<Atom>));
      }
    }
    return b;
  }
  ...
  void cohort_shutdown_cleanup() {
    ...
    if (b) {
      b->~hazptr_obj_cohort();
      // Allocator().deallocate((uint8_t*)b,
      // sizeof(folly::hazptr_obj_cohort<Atom>));
      Allocator().deallocate(reinterpret_cast<uint8_t*>(b),
                             sizeof(::folly::hazptr_obj_cohort<Atom>));
    }
  }
  ...
  size_t size_{0};
  size_t max_size_{0};
  mutable Atom<::folly::hazptr_obj_cohort<Atom>*> cohort_{nullptr};
};
