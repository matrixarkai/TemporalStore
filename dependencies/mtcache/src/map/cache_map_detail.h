...
class ValueHolder<
 ...
 public:
  ...
  ValueHolder(std::piecewise_construct_t, Arg&& k, Args&&... args) {
    ...
    item_ =
        reinterpret_cast<value_type*>(Allocator().allocate(sizeof(value_type)));
    new (item_) value_type(std::piecewise_construct,
                           std::forward_as_tuple(std::forward<Arg>(k)),
                           std::forward_as_tuple(std::forward<Args>(args)...));
  }
  ...
  ~ValueHolder() {
    if (owned_) {
      ...
      Allocator().deallocate(reinterpret_cast<uint8_t*>(item_),
                             sizeof(value_type));
    }
  }
...
class HazptrTableDeleter {
 ...
 public:
  explicit HazptrTableDeleter(size_t count) : count_(count) {}
...
class NodeT : public ::folly::hazptr_obj_base_linked<
 ...
 public:
  ...
  explicit NodeT(::folly::hazptr_obj_cohort<Atom>* cohort, NodeT* other)
      : item_(other->item_) {
    init(cohort);
  }
...
class alignas(64) BucketTable {
 public:
  ...
  BucketTable(size_t initial_buckets, float load_factor, size_t max_size,
              ::folly::hazptr_obj_cohort<Atom>* cohort)
  ...
  bool insert(Iterator& it, const K& k, InsertType type, MatchFunc match,
              ::folly::hazptr_obj_cohort<Atom>* cohort, Args&&... args) {
    return doInsert(it, k, type, match, nullptr, cohort,
                    std::forward<Args>(args)...);
  }

  template <typename MatchFunc, typename K, typename... Args>
  bool insert(Iterator& it, const K& k, InsertType type, MatchFunc match,
              Node* cur, ::folly::hazptr_obj_cohort<Atom>* cohort) {
    return doInsert(it, k, type, match, cur, cohort, cur);
  }

  // Must hold lock.
  void rehash(size_t bucket_count, ::folly::hazptr_obj_cohort<Atom>* cohort) {
    ...
    for (size_t i = 0; i < oldcount; i++) {
      ...
      for (; node != lastrun;
        ...
        auto newnode =
            reinterpret_cast<Node*>(Allocator().allocate(sizeof(Node)));
  ...
  void clear(::folly::hazptr_obj_cohort<Atom>* cohort) {
    size_t bcount = bucket_count_.load(std::memory_order_relaxed);
    Buckets* buckets;
  ...
  void max_load_factor(float factor) {
    // cache: Manually control write lock by map_trylock()
    // std::lock_guard<Mutex> g(m_);
    load_factor_ = factor;
    load_factor_nodes_ =
  ...
  // Attempt to acquire segment lock
  bool map_trylock() { return m_.try_lock(); }

  // Acquire segment lock
  void map_lock() { m_.lock(); }

  void map_unlock() { m_.unlock(); }

  Mutex* get_map_lock() { return &m_; }

 private:
  // Could be optimized to avoid an extra pointer dereference by
  // allocating buckets_ at the same time.
  class Buckets
      : public ::folly::hazptr_obj_base<Buckets, Atom,
                                        concurrenthashmap::HazptrTableDeleter> {
    using BucketRoot = ::folly::hazptr_root<Node, Atom>;
 ...
 public:
  class Iterator {
   public:
    ...
    ::folly::hazptr_array<3, Atom> hazptrs_;
 ...
 private:
  ...
  void getBucketsAndCount(size_t& bcount, Buckets*& buckets,
                          ::folly::hazptr_holder<Atom>& hazptr) {
    while (true) {
  ...
  template <typename MatchFunc, typename K, typename... Args>
  bool doInsert(Iterator& it, const K& k, InsertType type, MatchFunc match,
                Node* cur, ::folly::hazptr_obj_cohort<Atom>* cohort,
                Args&&... args) {
    auto h = HashFn()(k);
    // cache: Manually control write lock by map_trylock()
    ...
    while (node) {
      ...
      if (KeyEqual()(k, node->getItem().first)) {
        ...
        if (type == InsertType::DOES_NOT_EXIST) {
          ...
          // TODO(kai) consider move unlock back here
          // No need to lock until the end
          // g.unlock();
          // Release not under lock.
          node->release();
          // ConcurentHashMap::insert_or_assign is the only method that would
          // eventually call doInsert with an insert_type ANY. In that case,
          // we will return false if a key is found, so the caller can tell
          // whether an insert or assign operation is performed.
          if (type == InsertType::ANY) {
            return false;
          }
          return true;
        }
...
class alignas(64) ConcurrentHashMapSegment {
 ...
 public:
  ...
  bool insert(Iterator& it, Key&& k, Value&& v) {
    ...
    auto res =
        insert_internal(it, node->getItem().first, InsertType::DOES_NOT_EXIST,
  ...
  bool try_emplace(Iterator& it, Key&& k, Args&&... args) {
    ...
    return insert_internal(it, std::forward<Key>(k), InsertType::DOES_NOT_EXIST,
  ...
  bool emplace(Iterator& it, const KeyType& k, Node* node) {
    return insert_internal(it, k, InsertType::DOES_NOT_EXIST,
                           [](const ValueType&) { return false; }, node);
  }
  ...
  bool insert_or_assign(Iterator& it, Key&& k, Value&& v) {
    ...
    auto node = reinterpret_cast<Node*>(Allocator().allocate(sizeof(Node)));
    new (node) Node(cohort_, std::forward<Key>(k), std::forward<Value>(v));
    auto res = insert_internal(it, node->getItem().first, InsertType::ANY,
                               [](const ValueType&) { return false; }, node);
    // If an ASSIGNMENT op is performed, false will be returned here with the
    // updated behavior in BucketTable::doInsert. Thus node should no longer
    // be destructed here.
    return res;
  }
  ...
  bool assign(Iterator& it, Key&& k, Value&& v) {
    ...
    new (node) Node(cohort_, std::forward<Key>(k), std::forward<Value>(v));
    auto res =
        insert_internal(it, node->getItem().first, InsertType::MUST_EXIST,
                        [](const ValueType&) { return false; }, node);
    if (!res) {
      node->~Node();
      // Allocator().deallocate((uint8_t*)node, sizeof(Node));
  ...
  template <typename Key, typename Value>
  bool assign_if_equal(Iterator& it, Key&& k, const ValueType& expected,
                       Value&& desired) {
    // auto node = (Node*)Allocator().allocate(sizeof(Node));
    auto node = reinterpret_cast<Node*>(Allocator().allocate(sizeof(Node)));
    new (node)
    ...
    auto res = insert_internal(
        it, node->getItem().first, InsertType::MATCH,
        [&expected](const ValueType& v) { return v == expected; }, node);
    if (!res) {
      node->~Node();
      // Allocator().deallocate((uint8_t*)node, sizeof(Node));
      Allocator().deallocate(reinterpret_cast<uint8_t*>(node), sizeof(Node));
  ...
  template <typename MatchFunc, typename K, typename... Args>
  bool insert_internal(Iterator& it, const K& k, InsertType type,
                       MatchFunc match, Args&&... args) {
    return impl_.insert(it, k, type, match, cohort_,
                        std::forward<Args>(args)...);
  }
  ...
  // Attempt to acquire segment lock
  bool map_trylock() { return impl_.map_trylock(); }

  void map_lock() { impl_.map_lock(); }

  void map_unlock() { impl_.map_unlock(); }

  Mutex* get_map_lock() { return impl_.get_map_lock(); }

 private:
  ImplT impl_;
  ::folly::hazptr_obj_cohort<Atom>* cohort_;
};
